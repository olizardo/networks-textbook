...
nodes_temp <- data.frame(name = c("Bob", "Carol", "Ted", "Alice"), 
                         gender = c("Male", "Female", "Male", "Female"))
l_temp <- matrix(c(-0.5, 0.5, 0.5, 0.5, 0.5, -0.5, -0.5, -0.5), ncol = 2, byrow = TRUE)

# Plot 1: Time 1 (Bob & Carol date, Ted & Alice date)
g_temp1 <- tbl_graph(nodes = nodes_temp, 
                     edges = data.frame(from = c("Bob", "Ted"), to = c("Carol", "Alice")), 
                     directed = FALSE)
p_temp1 <- ggraph(g_temp1, layout = "manual", x = l_temp[,1], y = l_temp[,2]) +
  ...
  scale_color_manual(values = c("Male" = "dodgerblue2", "Female" = "hotpink")) +
  labs(title = "Time 1: Two Couples\n(Bob-Carol & Ted-Alice)") + theme_graph() + 
...
# Plot 2: Time 2 (After breakups, Carol & Ted date)
g_temp2 <- tbl_graph(nodes = nodes_temp, 
                     edges = data.frame(from = c("Carol"), to = c("Ted")), 
                     directed = FALSE)
p_temp2 <- ggraph(g_temp2, layout = "manual", x = l_temp[,1], y = l_temp[,2]) +
  ...
  geom_node_text(aes(label = name), nudge_y = 0.24, size = 5.5, fontface = "bold", color = "grey20") +
  scale_color_manual(values = c("Male" = "dodgerblue2", "Female" = "hotpink")) +
  labs(title = "Time 2: New Couple\n(Carol & Ted date)") + theme_graph() + 
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 11), legend.position = "none") +
  coord_cartesian(xlim = c(-0.8, 0.8), ylim = c(-0.8, 0.8), clip = "off")

# Plot 3: Time 3 (The Avoided Tie completing the 4-cycle loop)
edges_temp3 <- data.frame(from = c("Carol", "Bob"), to = c("Ted", "Alice"), type = c("Time 2", "Avoided"))
...
p_temp3 <- ggraph(g_temp3, layout = "manual", x = l_temp[,1], y = l_temp[,2]) +
  geom_edge_link(aes(color = type, linetype = type), width = 2) +
  scale_edge_color_manual(values = c("Time 2" = "steelblue", "Avoided" = "firebrick")) +
  scale_edge_linetype_manual(values = c("Time 2" = "solid", "Avoided" = "dashed")) +
  geom_node_point(aes(color = gender), size = 12) +
  geom_node_text(aes(label = name), nudge_y = 0.24, size = 5.5, fontface = "bold", color = "grey20") +
  scale_color_manual(values = c("Male" = "dodgerblue2", "Female" = "hotpink")) +
  labs(title = "Time 3: The Avoided Tie\n(Bob-Alice: leftover taboo)") + theme_graph() + 
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 11), legend.position = "none") +
  coord_cartesian(xlim = c(-0.8, 0.8), ylim = c(-0.8, 0.8), clip = "off")


# 3. McMillan's same-sex friendship code (Girl Code / Bro Code)
nodes_peer <- data.frame(name = c("Luke", "Emma", "Mark", "Chloe"), 
                         gender = c("Boy", "Girl", "Boy", "Girl"))
l_peer <- matrix(c(-0.5, 0.5, 0.5, 0.5, -0.5, -0.5, 0.5, -0.5), ncol = 2, byrow = TRUE)

# Left: Mixed-sex romantic relationship Luke-Emma, same-sex friendships Luke-Mark and Emma-Chloe
edges_peer1 <- data.frame(from = c("Luke", "Luke", "Emma"), to = c("Emma", "Mark", "Chloe"), type = c("Romantic", "Friend", "Friend"))
...
# Middle: Closure: Emma befriends Mark, Luke befriends Chloe (double-dating)
edges_peer2 <- data.frame(from = c("Luke", "Luke", "Emma", "Emma", "Luke"), to = c("Emma", "Mark", "Chloe", "Mark", "Chloe"), type = c("Romantic", "Friend", "Friend", "Friend", "Friend"))
...
# Right: Breakup: Luke and Emma break up. Emma cannot date Mark (friend's ex). Luke cannot date Chloe (friend's ex).
# Results in two disjoint chains, preventing 4-cycle!
edges_peer3 <- data.frame(from = c("Luke", "Emma"), to = c("Mark", "Chloe"), type = c("Friend", "Friend"))
...
p_conf <- ggraph(g_conf, layout = "circle") +
  ...
  geom_node_point(aes(color = gender), size = 12) +
  geom_node_text(aes(label = name), nudge_y = 0.25, size = 6, fontface = "bold", color = "grey20") +
  scale_color_manual(values = c("Male" = "dodgerblue2", "Female" = "hotpink")) +
...
```
- **Time 1**: Bob dates Carol, while Ted dates Alice.
- **Time 2**: After breakups, Carol and Ted start dating.
- **Time 3**: Should Bob and Alice date? If they do, they complete the forbidden **4-cycle loop** and both take "leftovers." So they avoid each other!
</center>
...
p_jefferson_spanning
```
- Forcing Bob and Alice to look outward stretches the tree!
</center>
:::
:::
...
1. **Phase 1**: **Luke** (Boy) and **Emma** (Girl) are dating (dotted), while Luke is same-sex friends with **Mark** (Boy), and Emma is same-sex friends with **Chloe** (Girl).
2. **Phase 2**: Through double-dating (triadic closure), Luke spends time with Chloe, and Emma spends time with Mark, establishing friendly ties across the group.
3. **Phase 3 (Breakup)**: Luke and Emma split. Same-sex **friendship loyalty** prevails. Mark will **never** date Emma (his friend Luke's ex), and Chloe will **never** date Luke (her friend Emma's ex). The 4-cycle is blocked, fracturing the group into two open chains!
