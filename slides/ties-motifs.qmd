...
#| fig-width: 5.5
#| fig-height: 5.5
gr_trans <- create_empty(3, directed = TRUE) %>% 
  bind_edges(data.frame(from = c(1, 1, 2), to = c(2, 3, 3))) %>% 
  mutate(name = toupper(letters[1:3]))
ggraph(gr_trans, layout = "manual", x = x_triad, y = y_triad) +
  geom_edge_link(color = "steelblue", edge_width = 1.5,
                  arrow = arrow(length = unit(4, 'mm'), type = "closed"), end_cap = circle(6, 'mm')) +
  geom_node_point(size = 20, color = "tan2") +
  geom_node_text(aes(label = name), size = 8, color = "white") +
  theme_graph() + ggtitle("Transitive Hierarchy AAA(I/O)") + coord_cartesian(clip = "off") +
...
#| fig-width: 5.5
#| fig-height: 5.5
gr_cen_ex <- create_empty(3, directed = TRUE) %>% 
  bind_edges(data.frame(from = c(1, 2, 2, 3), to = c(2, 1, 3, 2))) %>% 
  mutate(name = toupper(letters[1:3]))
ggraph(gr_cen_ex, layout = "manual", x = x_triad, y = y_triad) +
  geom_edge_parallel(color = "steelblue", edge_width = 1.25,
                 arrow = arrow(length = unit(4, 'mm'), type = "closed"), end_cap = circle(6, 'mm'), sep = unit(4, 'mm')) +
  geom_node_point(size = 20, color = "tan2") +
  geom_node_text(aes(label = name), size = 8, color = "white") +
  theme_graph() + ggtitle("Reciprocal Triad MMN") + coord_cartesian(clip = "off") +
  theme(plot.title = element_text(size = 11, face="bold", hjust=0.5))
```
:::
...
#| fig-width: 12
    ...
    # Define plotting function to save slide code space (perfect triangle coordinates)
