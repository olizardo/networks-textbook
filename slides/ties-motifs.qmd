...
## Network Growth: Two Separate Systems

:::: {.columns align=center}

::: {.column width="45%"}
::: {.incremental}
- Networks grow by linking different types of ties across previously disconnected systems.
- Consider two separate family trees:
  - **Family 1**: Parent 1, You, Sis (anti-symmetric parent-child ties, symmetric sibling tie).
  - **Family 2**: Parent 2, Riley, Bro (anti-symmetric parent-child ties, symmetric sibling tie).
- You and Riley fall in love!
  - This creates an **asymmetric romantic tie** (red dashed arrow) between the two systems.
:::
:::
...
- This single event immediately triggers a cascade of new "in-law" ties:
  - **Symmetric Sibling-in-Law**: You $\leftrightarrow$ Bro, Riley $\leftrightarrow$ Sis (blue dotted).
  - **Anti-Symmetric Parent-in-Law**: Parent 1 $
ightarrow$ Riley, Parent 2 $
ightarrow$ You (dashed gray).
- The two families are now merged!
:::
:::
...
# Add parent-in-law (1->5, 4->2) and sibling-in-law (2->6, 5->3)
edges_g2 <- data.frame(
...
- The baby adds a **cascade of new anti-symmetric kinship ties** to everyone in both systems:
  - Parent-child to You/Riley.
  - Grandchild to Parent 1/Parent 2.
  - Niece/Nephew to Sis/Bro.
...
# Add baby parent-child ties (2->7, 5->7), grandparent ties (1->7, 4->7), aunt/uncle (3->7, 6->7)
edges_g3 <- data.frame(
