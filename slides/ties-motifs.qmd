...
plot_tie_type <- function(nodes, edges, title, subtitle, directed = TRUE, has_cross = FALSE) {
  ...
  if (has_cross) {
    p <- p + annotate("text", x = 1.5, y = 0.94, label = "✕", color = "#e74c3c", size = 8, fontface = "bold")
  }
  ...
  p <- p + 
    ...
    coord_cartesian(xlim = c(0.6, 2.4), ylim = c(0.88, 1.12), clip = "off")
...
edges_rec <- data.frame(from = c(1, 2), to = c(2, 1), strength = c(0.06, 0.06), color = c("#2c3e50", "#2c3e50"), linetype = c("solid", "solid"))
edges_nonrec <- data.frame(from = 1, to = 2, strength = 0, color = "#2c3e50", linetype = "solid")
edges_anti <- data.frame(
  from = c(1, 2), 
  to = c(2, 1), 
  strength = c(0.06, 0.06), 
...
ggraph(gr_g1, layout = "manual", x = x, y = y) +
  geom_edge_parallel(aes(filter = type %in% c("parent", "romantic"), color = type, linetype = type),
                     arrow = arrow(length = unit(4.5, 'mm'), type = "closed"),
                     start_cap = circle(11, 'mm'), end_cap = circle(11, 'mm'),
                     sep = unit(5, 'mm'), edge_width = 1.25) +
  geom_edge_link(aes(filter = type == "sibling", color = type, linetype = type),
                 start_cap = circle(11, 'mm'), end_cap = circle(11, 'mm'), edge_width = 1.25) +
  ...
  geom_node_point(size = 36, color = "tan2") +
...
ggraph(gr_g2, layout = "manual", x = x, y = y) +
  geom_edge_parallel(aes(filter = type %in% c("parent", "inlaw"), color = type, linetype = type),
                     arrow = arrow(length = unit(4.5, 'mm'), type = "closed"),
                     start_cap = circle(11, 'mm'), end_cap = circle(11, 'mm'),
                     sep = unit(5, 'mm'), edge_width = 1.2) +
  geom_edge_link(aes(filter = type %in% c("sibling", "marriage"), color = type, linetype = type),
                 start_cap = circle(11, 'mm'), end_cap = circle(11, 'mm'), edge_width = 1.2) +
...
ggraph(gr_g3, layout = "manual", x = x, y = y) +
  geom_edge_parallel(aes(filter = type %in% c("parent", "inlaw", "baby"), color = type, linetype = type),
                     arrow = arrow(length = unit(4.5, 'mm'), type = "closed"),
                     start_cap = circle(11, 'mm'), end_cap = circle(11, 'mm'),
                     sep = unit(5, 'mm'), edge_width = 1.1) +
  geom_edge_link(aes(filter = type %in% c("sibling", "marriage"), color = type, linetype = type),
                 start_cap = circle(11, 'mm'), end_cap = circle(11, 'mm'), edge_width = 1.1) +
...
ggraph(gr_hierarchy, layout = "tree") +
  ...
  geom_node_point(size = 38, color = "tan2") +
  geom_node_text(aes(label = name), size = 5.0, fontface = "bold", color = "white") +
  theme_graph() + coord_cartesian(clip = "off")
```
:::
