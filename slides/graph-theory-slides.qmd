...
p <- p + geom_node_point(aes(x = x, y = y), size = 18, color = "tan2") 
p <- p + geom_node_text(aes(label = name), size = 8, color = "white")
p + theme_graph() + coord_cartesian(clip = "off") + coord_cartesian(clip = "off")
```
:::
...
::: {.incremental}
- **Empty graph**: The resulting graph when we delete all the edges in a graph ($G' = G - E$).
- **Ego graph**: The subgraph resulting from picking one node and deleting all nodes that are not adjacent to it.
  - The node's adjacent contacts are called its **alters**.
- **Null graph**: The subgraph resulting from deleting all nodes in the graph.
:::
