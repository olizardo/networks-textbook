...
- **Before** (Left):
  - Star graph of 4 nodes, with the center node highlighted in red.
- **After** (Right):
  - The center node is removed, leaving 3 isolated nodes (disconnected).
:::
:::
...
p_before <- ggraph(gr_k1, layout = "circle") +
  geom_edge_link(color = "steelblue", edge_width = 1) +
  geom_node_point(aes(color = ifelse(name == "1", "yes", "no")), size = 15) +
  scale_color_manual(values = c("yes" = "red", "no" = "tan2"), guide = "none") +
  geom_node_text(aes(label = name), size = 7, color = "white", fontface="bold") +
  theme_graph() + ggtitle("Before (With Cutpoint)") + coord_cartesian(clip = "off") +
  theme(plot.title = element_text(size = 11, face="bold", hjust = 0.5))

gr_k1_after <- gr_k1 |> filter(name != "1")
p_after <- ggraph(gr_k1_after, layout = "circle") +
  geom_node_point(size = 15, color = "tan2") +
  geom_node_text(aes(label = name), size = 7, color = "white", fontface="bold") +
...
- **Before** (Left):
  - Ring graph of 4 nodes, with nodes 1 and 2 (the cutset) highlighted in red.
- **After** (Right):
  - Removing nodes 1 and 2 leaves nodes 3 and 4 completely disconnected.
:::
:::
...
p_before <- ggraph(gr_k2, layout = "circle") +
  geom_edge_link(color = "steelblue", edge_width = 1) +
  geom_node_point(aes(color = ifelse(name %in% c("1", "2"), "yes", "no")), size = 15) +
  scale_color_manual(values = c("yes" = "red", "no" = "tan2"), guide = "none") +
  geom_node_text(aes(label = name), size = 7, color = "white", fontface="bold") +
  theme_graph() + ggtitle("Before (Minimum Cutset of 2)") + coord_cartesian(clip = "off") +
  theme(plot.title = element_text(size = 11, face="bold", hjust = 0.5))

gr_k2_after <- gr_k2 |> filter(!name %in% c("1", "2"))
...
- **Before** (Left):
  - Complete $K_4$ graph of 4 nodes, with nodes 1, 2, and 3 highlighted in red.
- **After** (Right):
  - Removing 3 nodes leaves a single isolated node (trivial graph).
:::
:::
...
p_before <- ggraph(gr_k3, layout = "circle") +
  geom_edge_link(color = "steelblue", edge_width = 1) +
  geom_node_point(aes(color = ifelse(name %in% c("1", "2", "3"), "yes", "no")), size = 15) +
  scale_color_manual(values = c("yes" = "red", "no" = "tan2"), guide = "none") +
  geom_node_text(aes(label = name), size = 7, color = "white", fontface="bold") +
  theme_graph() + ggtitle("Before (Minimum Cutset of 3)") + coord_cartesian(clip = "off") +
  theme(plot.title = element_text(size = 11, face="bold", hjust = 0.5))

gr_k3_after <- gr_k3 |> filter(!name %in% c("1", "2", "3"))
p_after <- ggraph(gr_k3_after, layout = "circle") +
  geom_node_point(size = 15, color = "tan2") +
  geom_node_text(aes(label = name), size = 7, color = "white", fontface="bold") +
  theme_graph() + ggtitle("After (Disconnected/Trivial)") + coord_cartesian(clip = "off") +
  theme(plot.title = element_text(size = 11, face="bold", hjust = 0.5))
