...
The matrix containing the eigenvectors corresponding to the adjacency matrix in @tbl-adjmat are shown in @tbl-eigen-1. The corresponding eigenvalues are shown as the diagonal entries of the matrix shown in @tbl-eigen-2. We can verify that arranging the nodes in the graph shown in @fig-eig according to the (absolute) value of the scores from the first column of @tbl-eigen-1. Once again, because we only care about the rank and not the specific number, we divide the scores in the first column of @tbl-eigen-1 by the maximum observed (absolute) value. As shown in @tbl-eig-final-2, this results in scores and a rank ordering identical to that which we found via our iterative procedure. 
