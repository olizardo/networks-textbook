---
title: "Applied Matrix Operations"
---
...
As we noted in @sec-mattrans, it is always possible to multiply any matrix times its transpose. Well, the adjacency matrix of a network ($\mathbf{A}$) like @tbl-pows-1 is a matrix. That means it call always be multiplied times its transpose ($\mathbf{A}^T$), resulting in some other matrix $\mathbf{B}$, of the same dimensions as the original adjacency matrix:
