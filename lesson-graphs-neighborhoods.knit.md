...
::: {.cell-output-display}
![Another simple graph.](lesson-graphs-neighborhoods_files/figure-html/fig-simple2-1.png){#fig-simple2 width=672}
:::
:::
...
Just like in undirected (simple) graphs, each node in a directed graph has a **node neighborhood**. However, because now each node can be the **source** or **destination** for an asymmetric edge, this means that we have to differentiate the neighborhood of a node depending on whether the node is the **sender** or the **recipient** of a given link. 

So, we say that a node *j* is an **in-neighbor** of a node *i* if there is a directed link with *j* as the source and *i* as the destination node. For instance, in @fig-directed, *E* is an in-neighbor of *C*, because there's an asymmetric edge with *E* as the source and *C* as the destination. 
