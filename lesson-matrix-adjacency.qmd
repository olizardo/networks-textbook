...
For example, asking "Is A friends with A?" does not make much sense, but there are rare cases when it does, such as when A is a group rather than an individual, and the relationship we are looking at might occur both within and between groups. As we saw in @sec-graphtheory, these are called **reflexive-ties** or **loops**. But if the network is represented as a **simple graph**, it should contain no loops.
...
In the resulting matrix, A to B and B to A each have a 1 listed, while G to D and G to F also have a value of 1. However, the cells corresponding to F to G and D to G each have a value of 0 because the ties are unreciprocated. These unreciprocated ties make the resulting matrix asymmetric. The two halves across the diagonal are no longer mirror images, but contain different entries.
