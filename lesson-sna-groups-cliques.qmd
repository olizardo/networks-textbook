...
Cliques come in different sizes. A clique of size four is a **maximally complete subgraph** with four nodes. A complete subgraph of order five would yield a clique of size five and so forth. A subgraph is **maximal** for a given property (like being complete) if adding one more node to the subgraph gets rid of the property. For instance, a maximally complete subgraph of order four (a clique of size four) means that adding on more nodes to the subgraph would make its density drop below one, and thus the subgraph of order five is no longer complete. 
...
@fig-clique1 shows a graph containing four cliques of size 4, with nodes belonging to these cliques highlighted in different colors. Nodes $\{A, B, C, D\}$ belong to one clique, nodes $\{E, F, G, H\}$, to another, nodes $\{I, J, K, L\}$ to yet another clique, and nodes $\{M, N, O, P\}$ to a final clique of size four. 
...
When studying a social network, we may be interested in determining how many cliques it contains. This means *enumerating* the full set of cliques above a certain minimum clique size. For instance, the total number of cliques of size four or larger in the graph shown in @fig-clique3 goes as follows:
...
@tbl-cliquemat shows the **Clique Affiliation Matrix** ($C$), with people in the rows (*i*) and all cliques of size four or larger in @fig-clique3 that we listed previously (*j*) in the columns [@everettborgatti98]. Each cell entry $C_{ij}$ in the clique affiliation matrix is set to one if node *i* belongs to clique *j*. Otherwise, it is set to zero. 

As we can see, some nodes (like node *A*) belong to only one clique, but other nodes, like node *D* belongs to six cliques! This is consistent with the idea that while some people belong to just a few groups, others are over-committed and belong to multiple groups. @fig-cliqueaff shows the corresponding bipartite graph displaying the clique affiliations in the network. People are shown as circles, and cliques are shown as triangles.
...
Just like the regular affiliation networks we saw in @sec-affiliation, we can compute the matrix product of the clique affiliation matrix $C$ and its transpose, $C^T$. This gives us the **clique co-membership matrix** ($M$). 
...
The off-diagonal cells of the clique co-membership matrix tell us the number of common clique nodes *i* and *j* share, and the diagonal cells tell us the number of cliques that each node belongs to. This is shown in @tbl-cliquecom.
...
The off-diagonal cells of the clique overlap matrix tell us the number of common members cliques *i* and *j* share, and the diagonal cells tell us the size of each clique. This is shown in @tbl-cliqueover. Note that clique three is an **isolate** in the clique graph, as it has no members in common with any other cliques.

From this information, we can construct what @everettborgatti98 calls the **Clique Graph**. This is a weighted graph with cliques as the nodes and the edges weighted by the number of people shared by each clique. The clique graph corresponding to the clique affiliation matrix in @tbl-cliquemat is shown in @fig-cliquegraph.
...
Can you see another subset of nodes in the graph that also form a 2-clique?^[Hint: take a look at the "kite" subgraph formed by nodes $\{I, J, K, L\}$]
...
Of course, it would be silly to keep on going with larger values of n larger than three, since the subgraphs so defined would be so loosely connected as to no longer count as groups [@luce50, p. 176]. That's why, when defining n-cliques, values of $n$ in the range of 2 or 3 are most commonly used.
