# Equivalence and Similarity {#sec-equivalence}
...
There are two main ways to partition nodes into equivalence classes. The first is based on the idea that two nodes occupy the same position if they have the same connectivity patterns to other nodes in the graph. This is called **structural equivalence** [@lorrainwhite71]. 

The second is based on the idea that two nodes are equivalent if they are connected to people who are themselves equivalent, even if those people are not literally the same. This is called **regular equivalence** [@whitereitz83]. 
