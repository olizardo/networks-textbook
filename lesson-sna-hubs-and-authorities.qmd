...
Multiplying the transpose of the adjacency matrix times the all ones vector gives us the row sums of the transpose, which is the same as the column sums of the original adjacency matrix (which is how we calculate in-degree centrality; see @sec-degcent).
...
#|   - ""
  ...
  # Calculate final HITS scores iteratively
  hits_iteration <- function(A, tol = 1e-6, max_iter = 100) {
    n <- nrow(A)
    h <- rep(1, n)
    names(h) <- colnames(A)
    u <- rep(1, n)
    ...
    for (k in 1:max_iter) {
      ...
      if (max(abs(h - h_prev)) < tol && max(abs(u - u_prev)) < tol) {
        break
      }
      h_prev <- h
      u_prev <- u
    }
    return(list(h_step2 = v2_norm, h = h, u = u, k = k))
  }
  res <- hits_iteration(A)
  hit.res <- list(
    h = res$h_step2,
    u = res$u,
    h_final = res$h,
  ...
  kbl(round(a/max(a), 2), format = "html", align = "c", col.names = NULL) %>% 
        column_spec(1, bold = TRUE) %>% 
        kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
  kbl(round(hit.res$h, 2), format = "html", align = "c", col.names = NULL) %>% 
        column_spec(1, bold = TRUE) %>% 
        kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
