...
Note also that, in the person overlap matrix, (in contrast to the usual adjacency matrix), there are valid entries along the diagonal cells ($o^P_{ii}$). These cells now record the *total number of memberships* that the node corresponding to that row (or column) has. Which we ascertained by computing the node centralities in the original affiliation matrix using \@ref(eq:aff1). You can see that the vector of degree centralities shown in @tbl-affdegcent1) is the same as the vector formed by the diagonal entries in @tbl-comem).
...
Recall from the lesson on matrices that matrix multiplication is *not* commutative (if $A$ is a rectangular matrix, then $A \times A^{'} \neq A^{'} \times A$)), so equation \@ref(eq:goverlap) gives you a different answer than equation \@ref(eq:poverlap). The result is shown in @tbl-goverlap)
