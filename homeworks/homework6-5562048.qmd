# Homework VI: Centrality {.unnumbered}
...
```{r}
    a <- as.matrix(sort(round(harmonic_centrality(gr2, normalized = TRUE), 4), decreasing = TRUE))
    colnames(a) <- c("Norm. Closeness")
    kable(a, format = "html", align = "c", full_width = F)  %>% 
         column_spec(1, bold = TRUE) %>% 
         column_spec (1:2, border_left = T, border_right = T) %>%
         row_spec(1:n, extra_css = "border-bottom: 1px solid") %>%
...
```{r}
    a <- as.matrix(sort(round(harmonic_centrality(gr2, normalized = FALSE), 4), decreasing = TRUE))
...
```{r}
    r <- all_shortest_paths(gr, from = "A", to = "I")$res
    paths_names <- lapply(r, function(path) V(gr)[path]$name)
    a <- do.call(rbind, paths_names)
    kable(a, format = "html", align = "c", full_width = F)  %>% 
         kable_styling()
```
RBC(C) = `r sum(apply(a, 1, function(x) {"C" %in% x})) / nrow(a)`
...
Centralization = `r sum(max(d) - d) / ((n - 1)*(n - 2))`