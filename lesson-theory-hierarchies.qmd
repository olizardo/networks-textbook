...
It is important to distinguish between transitive, intransitive, and **vacuously transitive** triads in dominance networks.

If $A \rightarrow B$ and $B \rightarrow C$, but the tie $A \rightarrow C$ is missing (or points the other way, $C \rightarrow A$), the triad is **intransitive** (a violation of transitivity). 
However, what if $A \rightarrow B$, but $B$ does *not* direct a tie to $C$? In this case, the precondition for transitivity ($A \rightarrow B$ AND $B \rightarrow C$) is not met. The triad cannot violate transitivity because the necessary chain doesn't exist. This configuration is called **vacuously transitive**. It does not actively demonstrate transitivity, but it doesn't violate it either.
...
ggraph(g, layout = 'manual', x = x, y = y) +
  ...
  theme_graph() + coord_cartesian(clip = "off") + theme(legend.position = "none")
```
...
ggraph(g, layout = 'manual', x = x, y = y) +
  ...
  theme_graph(base_family = "sans") + coord_cartesian(clip = "off") + theme(
...
The interaction-based model of hierarchy formation proposes that social hierarchies emerge not solely from inherent individual attributes but also, significantly, from the sequence and outcomes of past status competitions or **agonistic encounters**. This model contrasts with an attribute-based model, which posits that hierarchy is a direct reflection of your underlying qualities, such as size, age, or strength [@chase82a]. Instead, the interaction-based model emphasizes how local network processes, specifically the results of competitive interactions, influence an actor's future position in the social structure.
