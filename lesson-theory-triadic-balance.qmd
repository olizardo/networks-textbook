...
Because Balance Theory is a social-psychological theory, sentiment networks are built from **valenced ties**, which can be positive (e.g., liking) or negative (e.g., disliking). The theory states that unbalanced configurations within a **triad** (which as we saw in @sec-triads is a three-person social network configuration) consisting of P, an object O, and a third party Q produce "tension" for the focal node (P). This tension creates a psychological drive for these configurations to transition towards balanced ones either on the part of P or the others.
...
- Example: Imagine that you (P) have an rival (O) who dislikes another classmate (Q). It makes sense for you to extend a positive sentiment to the rival of your rival, as that means that now both you can form an alliance against them.
- Mathematical Representation: (-1) * (-1) * (+1) = +1.
- Sign Pattern: P-O (-) , O-Q (-) , P-Q (+). This triad has two negative links; thus, it is balanced.

3. **"A friend of an enemy is an enemy"**:
...
4. **"An enemy of a friend is an enemy"** :
...
#| fig-width: 8
    ...
    plot.triad <- function(edge_signs, title) {
    ...
    #Zero negative links: (+), (+), (+)
    plot.triad(c("+", "+", "+"), "A friend of a friend\nis a friend")
    #Two negative links: (-), (-), (+)
    plot.triad(c("-", "-", "+"), "An enemy of an enemy\nis a friend")
    #Two negative links: (-), (+), (-)
    plot.triad(c("-", "+", "-"), "A friend of an enemy\nis an enemy")
    #Two negative links: (+), (-), (-)
    plot.triad(c("+", "-", "-"), "An enemy of a friend\nis an enemy")
```
...
2. **"An enemy of a friend is a friend"**:
...
3. **"A friend of an enemy is a friend"**:
...
- Example: Imagine (the horror) that you (P) and your archnemesis (O) are attracted to the same person (Q). This would definitely create tension and discomfort (but may also be a good plot line for a YA story). This configuration of triadic sentiments is depicted in @fig-unbalanced-3.
- Mathematical Representation: P-O (-) * O-Q (+) * P-Q (+) = (-1) * (+1) * (+1) = -1.
...
#| fig-width: 8
    #One negative link: (+), (+), (-)
    plot.triad(c("+", "+", "-"), "1. A friend of a friend\nis an enemy")
    #One negative link: (+), (-), (+)
    plot.triad(c("+", "-", "+"), "2. An enemy of a friend\nis a friend")
    #One negative link: (-), (+), (+)
    plot.triad(c("-", "+", "+"), "3. A friend of an enemy\nis a friend")
    #Three negative links: (-), (-), (-)
    plot.triad(c("-", "-", "-"), "4. An enemy of an enemy\nis an enemy")
```
