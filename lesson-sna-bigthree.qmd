...
A star graph (a kind of **tree graph** as discussed in @sec-trees) is a graph containing a **central** or **inner** node (in @fig-star, node *A*), which is connected to all the other nodes in the graph, called the **satellite** or **outer** nodes (in @fig-star, nodes *B* through *F*). These nodes, in contrast, have only *one* connection and that is to the central node, none among themselves. 
