...
### Multiplying a Matrix Times its Transpose
...
It turns out that matrix powers was one of the earliest applications of formal social network analysis in the social sciences, discovered about the same time by mathematicians and social psychologists Luce and Perry [@luce49] and Festinger [@festinger49]. The basic idea is that when we obtain the **powers of an adjancency matrix** the resulting matrix has an intuitive interpretation in terms of indirect connections between people (see #sec-indirect), which gives us a sense of how strongly related in a formal sense pairs of nodes in the graph are. Let's see an example.
...
@tbl-pows-1 shows the adjacency matrix ($\mathbf{A}$) corresponding to the "hangout" network in @fig-rel-1. @tbl-pows-2 shows the entries in $\mathbf{A}^2$ computed like we did earlier in the examples shown in @tbl-pow. What are the meaning of the entries in each cell of @tbl-pows-2? Well for the *off-diagonal* entries, the numbers in each cell tell us the number of *indirect connections* (specifically the number of *walks*; see @sec-indirect) of *length two* between each pair of nodes. So, for instance, we learn that there are two walks of length two joing nodes $A$ and $B$ and one walk of length two joining nodes $E$ and $J$.

The *diagonal* cells, on the other hand, give us the number of indirect connections of length two that *originate* (and *end*, since the graph is undirected and the matrix is symmetric) in that node, which is an indirect version of degree centrality [@katz53]. So, for instance, we learn that three walks of length two originate from node *A* and four walks of length two originate from node *C*. 

@tbl-pows-3 shows the corresponding entries for $\mathbf{A}^3$. What do *these* numbers mean? Well, you may have guessed. For the of-diagonal cells they are the number of walks of length three linking each pair of nodes, and for the off-diagonal cells they are the number of walks of length three that originate and end in that node.  
