...
Accordingly, under Davis's model, a graph is **clusterable** if it contains no triads with exactly one negative tie. That is, the only triads that exist are the four balanced triads in @sec-tbalance, and the all-negative-ties triad. If this condition is met, the network can be partitioned into any number of distinct clusters, not just two. Just like in the Cartwright and Harary model, all the ties within a specific cluster are positive, but the ties between any two different clusters are entirely negative.
