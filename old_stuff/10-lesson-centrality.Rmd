...
The basic idea behind all centrality metrics is based on the **more/more principle**. This is the idea that *the higher any of the things above is for a node, then the more central that node is in the graph*. So a node with more nodes incident upon it (the size of the neighbor set; see section \@ref(sec:neighborhood)) should be more central. In the same way, a node who can **reach** many other nodes via the smallest shortest paths, should be more central (see section \@ref(sec:reachmat)). Finally, a node standing in the middle of the largest percentage of paths between other nodes should be more central (see section \@ref(sec:paths)), and so forth. We can refer to these as **network goods**. The more/principle has a basic measurement implication, which is that centrality should never *decrease* when a node gains access to more network goods (e.g., one an additional connection) in the network. The **more/more centrality principle** was described by legendary social network theorist [**Linton Freeman**](https://www.socsci.uci.edu/newsevents/news/2018/2018-08-23-freeman.php) in a classic paper [@freeman79.] 

Sociologically, the notion of centrality is tied to such concepts as "status," "prestige," "importance," "influence," "popularity," "reachability," "brokerage," and many more. So the more central nodes have higher status, are more popular, influential, broker more deals, and so forth. 

```{r star, fig.margin=TRUE, fig.cap="A star graph"}
    gr <- create_star(7) %>% 
...
These sociological concepts have to be kept distinct from any one centrality metric (remember the **three-step shuffle** in social network analysis discussed earlier), because the same concept can be measured by more than one centrality metric, and the same metric can point to many distinct sociological concepts, depending on the context, the type of social ties the network is constructed from, and the way either "goods" (e.g., information, advice, support, etc.) or "bads" (e.g., disease, gossip, violence etc.) "flow" through the network [@borgatti05]. 

# The "big three" centrality metrics
Linton @freeman79, in the aforementioned paper, defined the three classic centrality metrics, roughly corresponding to the extent that a nodes accumulates one of the three network goods mentioned above. So the **degree centrality** metric deal with nodes that have more edges directly incident upon them [@nieminen74]. The **closeness centrality** metric has to do with nodes that can reach more nodes via smallest **shortest paths** and thus accumulate as many of these paths in which they figure as the **origin node** as possible [@sabidussi66]. Finally, the **betweenness centrality** metric has to do with a node's accumulation of the largest share of shortest paths in which they intermediate between two other nodes, and thus featuring them as one of the **inner nodes** in the paths between others [@freeman77]. Other centrality measures (like **eigenvector centrality** to be discussed later) can be seen as generalizations or special cases of any of these three basic notions. 
...
Freeman showed that the three basic measures reach their theoretical *maximum* for the central node in a **star graph**, such as the one shown in Figure \@ref(fig:star). 

A star graph is a graph containing a **central** or **inner** node (in this case, node *A*), who is connected to all the other nodes in the graph, called the **satellite** or **outer** nodes. These nodes in contrast have only *one* connection and that is to the central node, none among themselves. 

Because of these restrictions, it is easy to see that if $G = (V, E)$ is a star graph of order $n$, then we know that that graph size $m = |E|$ (the size of the edge set), has to be $n-1$. 

# Degree Centrality

```{r undirected, fig.cap="An undirected graph.", fig.margin=TRUE, fig.height=10}
...
Once we have constructed the **adjancency matrix** for the network (**A**), then degree centrality is easy to calculate. As Equation \@ref(eq:degcent) for a given node *i* the degree centrality is given by summing the entries of its corresponding row.
...
For instance, if we were to use equation \@ref(eq:degcent) to calculate the degree centrality of each node from the symmetric adjancency matrix corresponding to the graph shown in Table \@ref(fig:undirected) then we would end up with the following degree centralities for each node:
...
```{r degcent}
    ...
    d <- degree(gr)
    d <- t(d)
    kbl(d, caption = "Degree centralities of nodes in an undirected graph.") 
```

## How to Read Equations for Matrix Operations
Before we continue, a note on something obvious. A lot of centrality measures are expressed as equations and these coud be hard to interpret initially. Depending on your background in math, you may or may not already know how to interpret Equation \@ref(eq:degcenteq). Essentially, the number at the bottom of the sigma is where to start. The number at the top of the Sigma symbol ($\Sigma$) is where to end. The equation to the left is the operation to be performed. Thus, one reads the Equation \@ref(eq:degcenteq) as, starting at column $j=1$, and ending at the last possible column $n$ ($n$ is simply the total number of rows in the matrix, $n$ means to go to the final value in the matrix), add up all possible values of the cells designated by the row $i$ and column $j$ combination in matrix **A**. Thus, to calculate the degree centrality of each $j = a, b, c$ in the below matrix, each of the following calculations would be performed.
...
```{r directed, fig.cap="A directed graph.", fig.margin=TRUE, fig.height=8}
...
If we are talking about a directed graph, then there are two types of centralities that can be calculated. On the one hand, we may be interested in how central a node is in terms of *sociability* or *expansiveness* that is how many other nodes in the graph a given node **sends** links to. This is called the **outdegree centrality** of that node, written as $C_i^{OUT}$. As with the undirected case, this is computed by summing across the rows of the **asymmetric adjacency matrix** corresponding to the directed graph in question, using the same equation \@ref(eq:degcent) above:

\begin{equation}
  C_j^{OUT} = \sum_{i = 1}^{n}a_{ij}
  (\#eq:outdegcent)
\end{equation}
...
For instance, if we were to use equations \@ref(eq:outdegcent) and \@ref(eq:outdegcent) to calculate the outdegree and indegree centrality of each node from the asymmetric adjacency matrix corresponding to the graph shown in Figure \@ref(fig:directed), then we would up with the following centralities for each node:
...
```{r outindegcent}
    ...
    kbl(d, caption = "Out and Indegree centralities of nodes in a directed graph.") 
```
...
```{r closfig, fig.margin=TRUE, fig.cap="An undirected graph showing the node with the maximum closeness centrality (in red).", fig.width=12, fig.height=8}
  set.seed(76)
...
This information is captured in a network's geodesic **distance matrix**, as discussed in the lesson on matrices. For instance, take the graph shown in Figure \@ref(fig:closfig). The distance matrix for which is shown in Table \@ref(tab:distab). 
...
```{r distab}
  ...
  gr <- simplify(gr)
  d <- distances(gr)
  kbl(d, caption = "Geodesic distance matrix for an undirected graph.")
```
...
\begin{equation}
  C_i^{CLOS} = N\frac{1}{\sum_{i = j}^{n}d_{ij}}
  (\#eq:closcenteq)
\end{equation}

In equation \@ref(eq:closcenteq), the denominator is the sum of the sum (across columns) for each row in Table \@ref(tab:distab). We take the mathematical *inverse* (dividing one by the sum of distances) so that way, the smallest number comes out on top and the bigger number comes out of the bottom. We multiply the inverse sum of distances for each node by the constant *N* (the graph order) because dividing one by a big number results in very small numbers, which could be hard to interpret. 

Applying equation \@ref(eq:closcenteq) to the distance matrix shown in Table \@ref(tab:distab), results in the following closeness centrality scores for each node in Figure \@ref(fig:closfig).
...
```{r closcent}
  ...
  gr <- simplify(gr)
  d <- distances(gr)
  d <- t(round(length(d)*1/rowSums(d),1))
  kbl(d, caption = "Closeness centrality scores.")
```

The closeness centrality scores listed in Table \@ref(tab:closcent) agree with our informal impressions. Node *I* comes out at the bottom ($CC_I^{CLOS} = 5.6$), showing it to be the one with the least closenes centrality, given the relatively large geodesic distances separating it from the other nodes in the graph. Node *E* (marked red in Figure \@ref(fig:closfig)) comes out on top ($CC_E^{CLOS} = 8.9$), given its relative geodesic proximity to other nodes in the graph.

As we will see later, having closeness centrality information for nodes in a graph can be useful. For instance, if Figure \@ref(fig:closfig)) was a social network, and we wanted to spread an innovation or a new product among the actors in the fastest amount of time, we would want to give it to node *E* first. Note however that if something **bad** (like a disease) was spreading across the network, then it would also be very bad if actor *E* got it first!^[See the discussion [here](http://www.analytictech.com/networks/centaids.htm).]
...
```{r betfig, fig.cap="An undirected graph showing the node with the maximum betweenness centrality (in red)", fig.height=8, fig.width=12}
  set.seed(76)
  fr <- c(sample(toupper(letters[1:14]), 35, replace = TRUE))
  to <- c(sample(toupper(letters[1:14]), 35, replace = TRUE))
  edge.dat <- data.frame(fr, to)
...
For instance, let's say you were actor *K* in the network shown in Figure \@ref(fig:closfig), and you wanted to know who is the person that you depend on the most to communicate with actor *J*. Here dependence means that you are forced to "go through them" if *I* wanted to reach *N* via a shortest path. One way *K* could figure this out is by listing every shortest path having them as the **origin node** and having *N* as the **destination node**. After you have this list, you can see which of other other nodes shows up as an **inner node** (an intermediary or gatekeeper) in those paths the most times. 
...
So it looks like, looking at the second column of table \@ref(tab: pairdep), that *H* is the other actor that *J* depends on the most to reach *K*. A better way to quantify this, is to actually look at the *proportion* of paths linking *J* and *K* that a particular other node (like *H*) shows up in. Let's call this $p_{KJ}(H)$ which can be read as "the proportion of paths between *K* and *J* featuring *H* as an inner node." This is shown in the third column of Table \@ref(tab:pairdep) We can write this in equation form like this:

\begin{equation}
  p_{KJ}(H) = \frac{g_{KJ}(H)}{G_KJ} = \frac{3}{6} = 0.5
  (\#eq:pairdep)
\end{equation}

In equation \@ref(eq:pairdep), $g_{KJ}(H)$ is the number of shortest paths linking *K* and *J* featuring *H* as an inner node, and $g_{KJ}$ is the total number of paths linking *K* and *J*. @freeman80 calls this measure the **pair-dependency** of actor *K* on actor *H* to reach a given node *J*. In this case, $g_{KJ}(H) = 3$ and $g_{KJ} = 6$, which means that actor *K* depends on actor *H* for fifty percent of their shortest path access to *J*. Making *H* the actor in the network *J* depends on the most to be able to reach *J*. 

Generalizing this approach, we can do the same for each triplet of actors *i*, *j*, and *k* in the network. This is the basis for calculating betweenness centrality. That is, we can count the number of times *k* stands on the shortest path between two other actors *i* and *j*. We can all this number $g_{ikj}$. We can then divide it by the total number of shortest paths linking actors *i* and *j* in the network, which we refer by $g_{ij}$. Remember that two actors can be indirectly linked by *multiple shortest paths of the same length*, and that we can figure out how many short paths links pairs of actors in the network using the **shortest paths matrix**.

This ratio, written $\frac{g_{ikj}}{g_{ij}}$ then gives us the *proportion* of shortest paths in the network that have *i* and *j* as the end nodes and that feature *k* as an intermediary inner node. This can range from zero (no shortest paths between *i* and *j* feature node *k* as an intermediary) to one (all the shortest paths between *i* and *j* feature node *k* as an intermediary).
...
Computing this quantity for the graph shown in Figure \@ref(fig:closfig), yields the  betweenness centrality scores shown in Table \@ref(tab:betcent).

```{r betcent, echo = FALSE}
  set.seed(76)
  fr <- c(sample(toupper(letters[1:14]), 35, replace = TRUE))
  to <- c(sample(toupper(letters[1:14]), 35, replace = TRUE))
  edge.dat <- edge.dat[order(fr, to),]
  ...
  kbl(b, caption = "Betweenness centrality scores.")
```

The numbers in the Table can be readily interpreted as percentages. Thus, the fact that node *J* has a a betweenness centrality score of 10.8 tells us that they stand in about 11% of the shortest paths between pairs of nodes in the graph. Interestingly, as shown in Figure \@ref(fig:betfig), the node that ends up with the highest betweenness score is *L* ($C_L^{BET} = 16.4$), mostly due to the fact that node *I*, who has the lowest possible betweenness score of zero, depends on this node for access to every other actor in the network. 

Note also that the fact two different nodes end up being ranked first in terms of closeness and betweenness centrality in the same network (compare Figure \@ref(fig:closfig) and Figure \@ref(fig:betfig)), tells us that these are analytically distinct measures of node position. 

# Centralities in the Star Graph
Degree, Closeness, and Betweenness centralities have an interesting property that provides a conceptual connection between them. Consider the star graph of order nine shown in Figure \@ref(fig:starplot) with central node *A*. The degree, closeness, and betweenness centralities of the different nodes are shown in Table \@ref(tab:startab). 

```{r starplot, fig.margin = TRUE, fig.cap = "A star graph of order nine.", fig.width=10, fig.height=10}
    gr <- create_star(9, directed = FALSE, mutual = FALSE, mode = "out")
    V(gr)$name <- toupper(letters[1:length(V(gr))])
    p <- ggraph(gr, layout = 'kk') 
    p <- p + geom_edge_link(color = "steelblue", width = 1.25) 
    p <- p + geom_node_point(aes(x = x, y = y, color = c("a", rep("b", length(V(gr))-1))), size = 28)
...
```{r startab}
    ...
    kable(c.tab, format = "html", caption = "Centralities in a star graph.")
```
...
```{r eigendegtab}
    ...
    d <- degree(gr)
    d <- t(d[order(-d)])
    colnames(d) <- c(rep("Mag",3), "Red", "Blue", "Gray", rep("Tan", 16))
    kbl(d, caption = "Degree centralities of nodes in the graph below.") 
```
```{r eigenfig1, fig.margin=TRUE, fig.cap="An undireced graph with degree centrality indicated by node color."}
    fr <- c(rep(1, 5), rep(2, 5), rep(3, 5), rep(4, 3), rep(20, 3))
    to <- c(5:9, 10:14, 15:19, 1:3, 5, 21, 22)
    edge.dat <- data.frame(fr, to)
    node.dat <- data.frame(name = union(fr, to))
    node.color <- c(rep("magenta",3), "firebrick", "gray50", rep("tan1", 14), "dodgerblue2", rep("tan1", 2))
    gr <- tbl_graph(edges = edge.dat, nodes = node.dat, directed = FALSE)
    gr <- as_tbl_graph(simplify(gr))
    p <- ggraph(gr, layout = 'kk') 
    p <- p + geom_edge_link(color = "steelblue", width = 0.85) 
    p <- p + geom_node_point(aes(x = x, y = y), color = node.color, size = 6.5)
    p <- p + theme_graph()
    p
```

Consider, for instance, the graph shown in Figure \@ref(fig:eigenfig1). The corresponding **degree set** of the graph (by node color) is shown as Table \@ref(tab:eigendegtab). In that figure, if we go by the simple degree centrality measure, the nodes shown in magenta, are the best connected ones. Each of these nodes connects to six others. The nodes shown in tan are the least well connected; each of those connect to only one neighbor The node shown in gray is marginally better connected than the tan nodes, featuring two connections. Finally, the nodes in red and blue are tied for "in between" in terms of degree centrality; each connects to three others.

Yet, given the previous reflections, there's a lingering suspicion that the best connected node in the graph is actually the red node, because even though that node only has three connections, all three go to well connected people. Thus, the centrality "tie" with the blue node (based on degree) is misleading, if we weighed the ties of the red based on the degree of the nodes they connect to, they should count for more than the three ties the blue node has, which go to less well-connected people. 
...
```{r eigencentab}
    ...
    colnames(d) <- c(rep("Mag",3), "Red", "Blue", "Gray", rep("Tan", 16))
    kbl(d, caption = "Eigenvector centralities of nodes in the graph above") 
```
...
All of these reflections lead us to the conclusion that degree centrality may be an incomplete measure of node position, because it only takes into account the number of connection a node has, not who those connections are to. A more general version of centrality would capture that. 
...
How does it work? Imagine that we start with regular old degree. This gives us the degree distribution and node ranking shown in Table \@ref(tab:eigendegtab); for each neighbor they have, each node gets one degree "point." Now in the second step, we can give each node extra points for each of the *friends their first step friends have*. In the third step, we can give node even more extra points for each *friend that their first step friends' friends have*, but discount these points a bit, because friends of friends are less important than friends. Then we keep going and give each node points for each *friend that their second step friends of friends' friends have* but discount these even more because friends of friends of friends are obviously less important than friends of friends. We can continue doing this until we can't go anymore (we reach the mathematical limit of the network in terms of how many friends, of friends, of friends, etc. people can have given the graph order and size). In his classic paper, Bonacich showed that the eigendecomposition of the network's adjacency matrix (technically the **eigenvector** corresponding to the largest **eigenvalue** of the adjacency matrix with all positive numbers) give us these scores, without having to go through the whole friends, of friends, of friends, of friends, etc. thing. Matrix Algebra magic!

Table \@ref(tab:eigencentab) shows the eigenvector centrality scores for the network shown in Figure \@ref(fig:eigenfig1). To aid interpretation, eigenvector centrality scores are *normalized* so that the top node receives an eigenvector centrality of 1.0 and then other nodes receive smaller scores proportional to how close their eigenvector centrality score is to the top node. Nodes with the lowest eigenvector centralities in the graph receive scores closer to zero. 

As we can see by the scores listed in Table \@ref(tab:eigencentab) the eigenvector centrality procedure recovers our intuitions. The red node comes out on top  ($C_{Blue}^{EIG} = 1.0$), even though it does not have the biggest degree. This is followed by the three magenta nodes. Node that not all three of the magenta nodes have the same eigenvector centrality (even though they have the same the degree), one of them has a slightly larger value. Can you tell which one that is in  Figure \@ref(fig:eigenfig1)? Well it's the magenta node connected to the gray node, and the reason it ends up with a higher eigenvector centrality than the other magenta ones is because it gets extra centrality points from the gray nodes' friends!

In the same way, note that the four tan nodes connected to the magenta node who's connected to the gray node get a slightly larger eigenvector centrality score than the other tan nodes ($C_{Tan}^{EIG} = 0.35$). The reason is that these tan nodes get a few extra centrality points from the their indirect (two-step) connection to the gray node and their three-step connection to the gray node's contacts. Note also that the blue node, presumably "tied" with the red node in terms of degree, ends up in the diametrically opposite place from this node in terms of eigenvector centrality: in the bottom three ($C_{Blue}^{EIG} = 0.20$). The reason is that while the blue node, like the red node, has three connections two of those go to nodes with no other friends (so no extra friends of friends points), and the other one goes to a node that only has degree centrality equal to 2. So the blue node is the less well-connected if by that we mean being connected to connected others.

Finally note that two tan nodes receive the lowest eigenvector centrality scores ($C_{Tan}^{EIG} = 0.07$). As you may have guessed, those are the two tan nodes shown in the rightmost area of the network diagram. The reason for this is that in contrast to the other tan nodes, who are connected to more central magenta nodes, those are connected to the not not-very well-connected blue node (who's connected to the not very well-connected gray node), so they derive very little extra points from those connections.
