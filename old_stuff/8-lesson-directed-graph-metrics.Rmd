...
```{r dyad-census}
    ...
    kbl(dc, caption = "Dyad census of a directed graph", align = "c") 
```
 
The dyad census tells us that out of all the connected dyads in Figure \@ref(fig:direx1), four of them are mutual: $\{BD, CG, DE, FE\}$, and ten of them are asymmetric: $\{AC, AE, BC, DF, EG, DF, EG, FA, FB, FG\}$. 
...
Where the numbers in Table \@ref(tab:dyad-census) come from? Well, it turns out that they can be readily computed from the **asymmetric adjacency matrix** ($A$)of the corresponding directed graph. The one corresponding to Figure \@ref(fig:direx1) is shown as Table \@ref(tab:adj1).
...
```{r adj1}
    ...
    diag(A) <- "--"
    kbl(A, caption = "Asymmetric adjancency matrix of a directed graph.")
```
...
```{r dirtriads, fig.cap="All Sixteen Directed Triad Configurations", fig.width=14, fig.height=16, fig.margin=TRUE}
    ...
    p <- p + geom_node_point(aes(x = x, y = y), size = 17, color = "tan2") 
    p <- p + geom_node_text(aes(label = name), size = 8, color = "white")
    p9 <- p + theme_graph() + xlim(-.1,1.1) + ylim(-.1,1.1) + ggtitle("AAA(C)")
    ...
    p <- p + geom_node_point(aes(x = x, y = y), size = 17, color = "tan2") 
    p <- p + geom_node_text(aes(label = name), size = 8, color = "white")
    p10 <- p + theme_graph() + xlim(-.1,1.1) + ylim(-.1,1.1) + ggtitle("AAA(I/O)")
    ...
    p <- p + geom_node_point(aes(x = x, y = y), size = 17, color = "tan2") 
    p <- p + geom_node_text(aes(label = name), size = 8, color = "white")
    p11 <- p + theme_graph() + xlim(-.1,1.1) + ylim(-.1,1.1) + ggtitle("MAA(C)")
    ...
    p <- p + geom_node_point(aes(x = x, y = y), size = 17, color = "tan2") 
    p <- p + geom_node_text(aes(label = name), size = 8, color = "white")
    p12 <- p + theme_graph() + xlim(-.1,1.1) + ylim(-.1,1.1) + ggtitle("MAA(O)")
    ...
    p <- p + geom_node_point(aes(x = x, y = y), size = 17, color = "tan2") 
    p <- p + geom_node_text(aes(label = name), size = 8, color = "white")
    p13 <- p + theme_graph() + xlim(-.1,1.1) + ylim(-.1,1.1) + ggtitle("MAA(I)")
    ...
    p <- p + geom_node_point(aes(x = x, y = y), size = 15, color = "tan2") 
    p <- p + geom_node_text(aes(label = name), size = 7, color = "white")
    p14 <- p + theme_graph() + xlim(-.1,1.1) + ylim(-.1,1.1) + ggtitle("MMN")
...
```{r triad-census}
    ...
    kbl(tc, caption = "Triad census of a directed graph", col.names = c("Count")) 
```

Just like we did in the case of directed dyads, it is possible to go through a directed graph and count the number of directed triadic motifs. This is called the **triad census** and was developed by sociologists and mathematicians working together in the 1970s [@davis_leinhardt72]. For instance, if we were to perform the triad census on the graph shown in Figure \@ref(fig:direx1) we would end up with the counts shown in Table \@ref(tab:triad-census). For you reference, the sixteen triadic configurations are shown in Figure \@ref(fig:dirtriads).

# Graph Reciprocity
Recall from our discussion in the lesson of types of graphs, that asymmetric relations in a directed graph have an important property that symmetric relations in an undirected graph lack: **Reciprocity**. That is, connected dyads in directed graphs can be either **mutual** (with directed edges going in both directions) or **asymmetric** with (with directed edges going only in one direction). So for instance, in the graph shown in Figure \@ref(fig:direx1) the connected $BD$ dyad is mutual, but the $FB$ dyad is asymmetric. 
...
In some applications we may be interested in Figuring out the extent to which the *whole graph* is more or less likely to contain mutual as opposed to asymmetric dyads. This metric is called **Graph Reciprocity** and is usually written as $R(G)$. For instance, if we compare the graph shown in Figure \@ref(fig:direx1) to the one shown in Figure \@ref(fig:direx2) it seems like there's much more reciprocity going in Figure \@ref(fig:direx1) than in Figure \@ref(fig:direx2). How can we quantify it?

Well, the count of the different types of directed dyads provided by the dyad census in Table \@ref(tab:dyad-census) provide us with the information we need. Accordingly, if we wanted to quantify the *amount* of reciprocity in the graph shown in Figure \@ref(fig:direx1), we could do it by computing the following ratio [@krackhardt94]:
...
The dyad census corresponding to the directed graph shown in Figure \@ref(fig:direx2) is shown in Table \@ref(tab:dyad-census2). Can you figure out the graph reciprocity?
...
Consider for instance the graph shown in Figure \@ref(fig:direx1). The corresponding reachability matrix is shown as Table \@ref(tab:reachmat1)
...
Which tells us that while not a perfect $1.0$, Figure \@ref(fig:direx1) is still a pretty well-connected social network!
...
```{r asymmreachmat}
    ...
    gr <- graph_from_adjacency_matrix(r, mode = c("directed"))
    diag(r) <- "--"
    kbl(r, caption = "Reachability matrix for a asymmetric graph.")
    1 - reciprocity(gr, mode = "ratio")
```
...
Thus, if we think of the tree graph as representing a purely *hierarchical* structure, in which authority, influence, orders, and so forth only flow in one direction, then the addition of reciprocity into the structure can be seen as a relaxation of the hierarchy principle. In Figure \@ref(fig:rectree) *A* doesn't just influence or order *B* around in a unidirectional way like they do in Figure \@ref(fig:tree), *B* can "talk back", or exert influence upon *A* and so forth. So by adding asymmetric reciprocal links, we have made the graph less hierarchical. 
...
Once we have $M^r$ we can figure out $U^r$, the number of unilaterally connected dyads, using a variation of  equation \@ref(eq:asymm):
...
Applying equation \@ref(eq:hier), to the tree graph shown in Figure \@ref(fig:tree) gives us the expected value of $1.0$, but applying the same equation to the asymmetric graph shown in Figure \@ref(fig:rectree) gives us the value of $H(G) = 0.77$, which is pretty high, but also shows that this network structure is less hierarchical than the pure anti-symmetric tree. Overall, the closer to zero the $H(G)$ score is, the less hierarchical the structure. For instance, the graph hierarchy score corresponding to the graph shown in Figure \@ref(fig:direx1) is $H(G) = 0.47$, showing that this network is even less hierarchical than the one shown in Figure \@ref(fig:rectree).

# Graph Efficiency

```{r treemiss, fig.margin=TRUE, fig.cap="A tree graph that is missing one link.", , fig.width=6, fig.height=6}
    fr <- c("A", "A", "B", "B", "C", "C")
    to <- c("B", "C", "D", "E", "F", "G")
    edge.dat <- data.frame(fr, to)
    node.dat <- data.frame(name = union(fr, to))
...
Tree graphs like the one shown in Figure \@ref(fig:tree) have another interesting property. The number of links in a tree is *always* equal to the number of nodes minus one! That is, in a tree graph $E = N - 1$. You can go back to Figure \@ref(fig:tree) and check for yourself. We have seven nodes $N$ and six links $E$ and $6 = 7 -1$. That's neat!

This means that trees are networks that have the *minimum* number of links to keep them connected. If you remove even *one* node from a tree you create a connectivity gap in the structure, splitting the graph into separate components. Like the one shown in Figure \@ref(fig:treemiss). After removing the $BE$ link node $E$ becomes isolated from the rest. The same would happen (creating disconnected clumps) if we were to remove any other link from Figure \@ref(fig:tree).


# Advanced topic: Reciprocity in directed weighted graphs
One phenomenon we can better understand using weighted graphs is the idea of reciprocity. Take for instance Figure \@ref(fig:dirweighted), which shows a **directed weighted graph**, with the same set of nodes and edges as Figure \@ref(fig:directed). Like all directed graphs, we can see that there are three kinds of dyads: **mutual**, **asymmetric** and **null**). This could represent the social network created by the flow of directed communications (e.g., emails or text messages send *from* one person *to* another) in an office or work group.

In standard (unweighted), directed graphs, we tend to say that two nodes that are part of a mutual dyad have a reciprocal relationship. So if we were going by a binary (on or off) idea, of reciprocity, we would say that all the mutual dyads in Figure \@ref(fig:dirweighted), such as $AB$, $AF$, and $BD$ are reciprocal. But as we said, when it comes to the sort of interactions that are best captured in a weighted graph, reciprocity is a matter of more or less, not all or none. The weights ($w$) provide a richer picture of what's going on in the network. 

## Notation
Let us come up with some notation. In a directed weighted graph, the weight of the directed edge going from source node *A* to source destination node *B* is referred to as $w_{AB}$. In the graph shown in in Figure \@ref(fig:dirweighted), $w_{AB} = 17$. If the dyad is mutual, we would refer to the directed edge coming from source node *B* to destination node *A* as $w_{BA}$. As you can see, $w_{BA}=20$.

```{r dirweighted, fig.cap="A directed weighted graph.", fig.height = 5, fig.width = 9}
    set.seed(3489)
...
Are mutual dyads in a directed weighted graph necessarily "reciprocal"? The answer is no. For instance, in Figure \@ref(fig:dirweighted), the relationship between *B* and *D* seems to be very uneven; *B* sent thirty messages to *D* ($w_{BD} = 30$) but *D* only sends two messages back ($w_{BD} = 2$). So we would say that reciprocity definitely seems to be lacking in the $BD$ dyad. 
