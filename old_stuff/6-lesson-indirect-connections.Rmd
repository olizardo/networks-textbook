...
```{r paths1, fig.margin=TRUE, fig.cap="A undirected graph showing a path between nodes *A* and *B* (red edges)."}
    gr <- create_path(4)
    gr <- gr %>% 
...
Note that every path is actually a **subgraph** of the original graph as defined in the graph theory lesson)! Except that the subgraph that defines a path will always contain a sequence of vertices and edges connected in a line.
...
-   Finally, there may be *multiple paths* connecting the same pair of nodes. For instance, In Figure \@ref(fig:paths1) node *A* can also reach node *B* via the path $(AC, CE, EB)$ which is distinct from the one shown in red the Figure, but which also counts as a proper graph theoretic path (intervening nodes only appear twice in the listing, and the end nodes only appear once).
...
Note that once we understand the graph theory notion of **connectivity**, it becomes clear that the concept of **adjacency** is a special (limiting) case of connectivity: Two nodes are adjacent when there is a path of $\mathcal{l} = 1$ between them!

In a simple graph with no isolates, like that shown in Figure \@ref(fig:paths1), it is easy to see that all node pairs are connected via a path of some length.

# Connected and Disconnected Graphs
Look at Figure \@ref(fig:paths1) again. Are there any **disconnected** pairs of nodes in the graph? The answer is no. If you pick any pair of nodes, they are either directly connected, or indirectly connected to one another via a path of some length. So how can we get two nodes to be disconnected in a simple undirected graph? The answer is that there has to be some kind of "gap" splitting the graph into two or more pieces, so that some nodes can no longer reach some of the other ones. 

```{r connected1, fig.margin=FALSE, fig.cap="A connected versus disconnected graph."}
    ...
    p <- p + geom_node_point(aes(x = x, y = y), size = 10, color = "tan2") 
    p <- p + geom_node_text(aes(label = name), size = 4, color = "white")
    p1 <- p + theme_graph() + xlim(-1.2,1) + ylim(-2,2.1)
    ...
    p <- p + geom_node_text(aes(label = name), size = 4, color = "white")
    p <- p + theme_graph() 
    p2 <- p + xlim(-1.2,1) + ylim(-2,2.1)
    p <- p1 + p2 + plot_annotation(tag_levels = 'A', tag_prefix = 'G(', tag_suffix = ')') & theme(plot.tag = element_text(size = 18))
    p
```

For instance, if you compare the undirected graphs $G(A)$ and $G(B)$ shown in Figure \@ref(fig:connected1)  you can appreciate the difference between a **connected graph** and a **disconnected graph**. The difference between the graph on the left $G(A)$ and the graph on the right $G(B)$, is that the $G(B)$ graph is a subgraph of the $G(A)$ graph, in which the $DF$ edge has been removed (an **edge-deleted subgraph** according to a previous lesson): $G(B) = G(A) - DF$. But removing this edge has resulted in $G(B)$ being disconnected. There is no way we can trace a path connecting any of the vertices in the $\{F, G, H, I\}$ subset with any of the vertices in the $\{A, B, C, D, E\}$ subset. This is different from $G(A)$ where we can trace a finite path between any two vertices, even if it is very long. Thus, we can come up with a formal definition of **graph connectivity**:
...
When a graph goes from connected (like $G(A)$ in Figure \@ref(fig:connected1)) to disconnected (like $G(B)$ Figure \@ref(fig:connected1)) it is split into a number of subgraphs that are themselves connected. For instance, in the $G(B)$ shown in Figure \@ref(fig:connected1) the set nodes $\{A, B, C, D, E\}$ form a **connected subgraph** of the larger disconnected graph. A connected subgraph that is part of a larger disconnected graph is called a **component**.  In the $G(B)$ graph in Figure \@ref(fig:connected1) there are two components, one formed by the connected subgraph with node set $\{A, B, C, D, E\}$, and the other formed by the connected subgraph with node set $\{F, G, H, I\}$. 

```{r connected2, fig.fullwidth = TRUE, fig.cap="A series of connected and disconnected graphs displaying components and the giant component.", fig.height=4.5, fig.width=11}
    ...
    p1 + p2  + p3 +
      plot_annotation(tag_levels = 'A', tag_prefix = 'G(', tag_suffix = ')') & theme(plot.tag = element_text(size = 18))

```

It is possible to split a graph into multiple components not just two. For instance, take a look at graph $G(A)$ in Figure \@ref(fig:connected2). If were to generate a subgraph from $G(A)$ by deleting the red and purple edges, we would end up with the $G(B)$ graph. The $G(B)$ graph is disconnected, and it features *three* separate components (connected subgraphs). 

```{r connected3, fig.margin = TRUE, fig.cap="A connected graph.", fig.height=4, fig.width=3}
    gr <- create_empty(14, directed = FALSE)
    gr <- gr %>%
      bind_edges(data.frame(from = 1, to = c(2:5))) %>% 
      bind_edges(data.frame(from = 2, to = c(3:5))) %>% 
...
If were to delete just the red edges from $G(A)$ and keep the purple edge, we would end up with the $G(C)$ graph in in Figure \@ref(fig:connected2). Like the $G(B)$ graph, the $G(C)$ graph is also disconnected, but it is split into two not three components. Note that one of the connected components in $G(C)$ is way larger than the other one. The bigger connected component of graph $G(C)$ is of order ten, but the smaller component is of order four. When a disconnected graph is split into multiple components, the component containing the largest number of nodes (the connected subgraph of the highest order) is called **the giant component**.

## Bridges

```{r connected4, fig.margin = TRUE, fig.cap="Another connected graph.", fig.height=4, fig.width=3}
    gr <- create_empty(14, directed = FALSE)
    gr <- gr %>%
      bind_edges(data.frame(from = 1, to = c(2:5))) %>% 
      bind_edges(data.frame(from = 2, to = c(3:5))) %>% 
...
Note that we went from the connected graph shown as $G(A)$ in Figure \@ref(fig:connected2) by removing or deleting particular edges (like the red and purple ones in $G(A)$). These edges are clearly more important than the blue ones, because they are responsible for keeping the whole graph together as a connected structure. Edges whose removal results in a graph going from being connected to disconnected are so important that they have a special name: **bridges**.

Thus, in a graph the bridges are (smallest in terms of cardinality) set of edges whose deletion results in the graph being disconnected. For instance, in graph $G(A)$ from Figure \@ref(fig:connected1) the $DF$ edge is a bridge. In graph $G(A)$ from Figure \@ref(fig:connected2), on the other hand, any two of the threesome formed by the two red edges and the purple edge count as bridges. Deleting any two of these three edges disconnects the graph. 

Note that in contrast to $G(A)$ from Figure \@ref(fig:connected1), the graph $G(A)$ from Figure \@ref(fig:connected2) doesn't have any *one* edge that is *the* bridge. Instead, it takes more the deletion of more at least two edges from the red and purple set to disconnect it. For instance, if we were to remove just the purple edge, the $G(A)$ would still be connected, and it would look like Figure \@ref(fig:connected3), which is a connected graph. The same goes for removing just one of the red edges. If were to do that, we would end up with the graph shown in Figure \@ref(fig:connected4), which is also a connected graph.

<!--## Graph Diameter-->  
# Shortest Paths 
Consider the multiple ways node *B* can reach node *C* via a path in Figure \@ref(fig:paths1). One possibility is the path defined by the edge sequence $(BE, ED, DA, AC)$. Another possibility is the path defined by the sequence $(BE, ED, DC)$. Yet another possibility is the path given by the edge sequence $(BE, EC)$. What's the difference between these paths? Well, for the first one, $\mathcal{l}_{BC} = 4$, for the second one $\mathcal{l}_{BC} = 3$ and for the last one $\mathcal{l}_{BC} = 2$. The three paths are of different length, even though all three are proper graph-theoretic paths (they do not repeat inner nodes) and even though all three feature the same actors as the end nodes.^[Note that the length of the smallest *possible* shortest path between two non-adjacent vertices is always two.]
...
Sometimes, as with actors *A* and *B* in Figure \@ref(fig:paths1), there will be **multiple shortest paths** between two pairs of nodes, because we end up with two or more paths that are "tied" in length (and thus all count as "shortest" paths). So for nodes *A* and *B* Figure \@ref(fig:paths1) the two shortest paths connecting them are of length 3: $(AC, CE, EB)$ and $(AD, DE, EB)$.

Observe that, in Figure \@ref(fig:paths1) if *B* really wanted to reach *C* via the shortest path, they will always have to go through node *E*. When this is the case, we say that *E* *stands in the shortest path* between *B* and *C*. So *B* is highly dependent on *E* to reach *C*. This makes *E*'s position in the network particular important for *B*, because they play the role of *go-between* or **broker** between *B* and the other actors in the network.

Because of this connection to communication efficiency and inter-mediation, shortest paths figure prominently in various measures of node position, called **centrality measures** we will deal with later on. One such measure, called **betweenness centrality** is based on counting the *number of shortest paths* between every other pair of nodes that a given node stands on. 

```{r cycle, fig.margin=TRUE, fig.cap="A undirected graph showing a cyle beginning and ending with node A (red edges)."}
    gr <- create_path(4)
    gr <- gr %>% 
        mutate(name = c("A", "D", "E", "B")) %>% 
        bind_nodes(data.frame(name = "C")) %>% 
...
The length of the shortest path between two pair of non-adjacent vertices in the network can be thought of as the "degrees of separation" between them. This is also called the **geodesic distance** between two nodes. For instance, the shortest path between nodes *A* and *B* is of length three, so *A* is three degrees of separation away from *B*. As we saw in this example, there does not have to be only one shortest path between two nodes. Actors in the network can be connected via multiple distinct shortest paths as are *A* and *B* in Figure \@ref(fig:paths1).

# Cycles
Consider the edge sequence $(AD, DE, EC, CA)$ highlighted in red in Figure \@ref(fig:cycle). What's so special about it? Well, it looks like a path, because the inner nodes are only listed twice and so all the edges are unique. However, both the origin and destination nodes are the same!
...
```{r tree, fig.margin=TRUE, fig.cap="A tree graph is also a directed acyclic graph (DAG)."}
    gr <- create_tree(n = 13, children = 3) 
    p <- ggraph(gr, layout = 'tree')
    p <- p + geom_edge_link(color = "steelblue", edge_width = 0.8,
...
For instance, looking at Figure \@ref(fig:paths1), we could imagine a sequence of edges that started with one node and ended in another one without repeating edges, but repeating nodes. For instance, the sequence $(CA, AD, DC, CE, ED)$ has node *C* as the origin node and node *D* as the destination node, and all the edges in the sequence are unique. However, the origin node *C* and the destination node *D* also appear in the intervening chain, which means that they are listed three times. This fails the path test for this edge sequence. Sequences like this, featuring all unique edges but repeated nodes, are called **trails**.
...
```{r dirpath3, fig.margin=TRUE, fig.cap="A directed graph showing two nodes B and C, mutually reachable via a path of length 3.", fig.width= 8}
    fr <- c("A", "A", "B", "C", "C", "D", "D", "E", "E")
    to <- c("C", "D", "E", "D", "E", "E", "C", "B", "D")
...
Also like before, there can be directed paths of different lengths connecting pairs of nodes. For instance Figure \@ref(fig:dirpath2) shows a highlighted red path of length $l_{AB} = 4$ with origin node $A$ and destination node $B$ and edges $(AD, DC, CE, EB)$. In the case of $A$ and $B$, there is at least one other path of length 4 with origin node $A$ and destination node $B$. Can you see it? 

```{r dirpath4, fig.margin=TRUE, fig.cap="A directed graph showing two nodes B and C, mutually reachable via a paths of different lengths.", fig.width= 8}
    fr <- c("A", "A", "B", "C", "C", "D", "D", "E", "E")
    to <- c("C", "D", "E", "D", "E", "E", "C", "B", "D")
...
## Mutual Reachability
In directed graphs, some pairs of nodes can be **mutually reachable**. That is, there can be a directed path going from one node to the other, and vice versa. Figure \@ref(fig:dirpath3) shows an example of this case involving nodes $B$ and $C$. Starting with origin node $B$ we can get to $C$ via the highlighted purple path of length $l_{BC} = 3$ formed by the edges $(BE, ED, DC)$. In the same way, starting with node $C$, we can get to node $B$ via the separate, highlighted red path of length $l_{CB} = 3$ formed by the edges $(CD, DE, EB)$.

For mutual reachability between two nodes to happen in a directed graph, the two paths do not have to be the same length in fact, in the case of nodes $B$ and $C$ node $C$ can reach $B$ via path shorter than length 3. This is shown in Figure \@ref(fig:dirpath4), where we can see that $C$ can reach $B$ via the shortest path possible short of being directly connected $(l_{CB} = 2)$, formed by the edges $(CE, EB)$ (highlighted in red). On the other hand, $B$ cannot reach $C$ via a path shorter than 3. 

```{r dirpath5, fig.margin=TRUE, fig.cap="A directed graph showing two weakly connected nodes A and B.", fig.width= 8}
