---
title: "Lesson: Affiliation Networks"
date: "`r Sys.Date()`"
output: 
...
```{r bipartite, fig.margin=TRUE, fig.cap="A bipartite graph. Circles are people and triangles are the corporate boards they belong to", echo = FALSE, warning = FALSE, message=FALSE}
...
Figure \@ref(fig:bipartite) represents a network diagram of a bipartite graph where circles connect to triangles (with the shapes standing as labels for the two set of nodes). In the Figure, $V_1 = \{A, B, C, D, E\}$ and $V_2 = \{1, 2, 3, 4, 5\}$. The edge set $E$ is $\{A1, A2, B2, B3, C2, C4, D4, D3, E3, E5\}$.

One common example of two-mode networks that be represented using bipartite graphs in sociology are **corporate interlock networks** [@mizruchi83]. If Figure \@ref(fig:bipartite) represented such a network, we could think of the circles as members of the company's board, and the triangles are the board from each company. Because the same executive can be a member of more than one company's board, board member A is on the board of both companies 1 and 2, while board member B is on the board of companies 2 and 3. 
...
# Unipartite Projections of Bipartite Graphs {#sec:projection1}
...
```{r bicircles, fig.margin=TRUE, fig.cap="A unipartite graph. People are linked if they serve in the same company board", echo = FALSE, warning = FALSE, message=FALSE}
...
```{r bitriangles, fig.margin=TRUE, fig.cap="Another unipartite graph. Boards are linked if they share members.", echo = FALSE, warning = FALSE, message=FALSE}
...
Thus, we could, as shown in Figure \@ref(fig:bicircles), create a graph that shows board members who know each other because they work at the same company. The resulting (simple, undirected) graph shows that board members A, B, and C all know each other as a result of serving in the board of company 2 together.

Likewise, we can transform the bipartite graph into a simple unipartite graph that captures companies that share board members. Company 2 is thus connected to Companies 1 (because of person A), 3 (because of person B), and 4 (because of person 5). This is shown in Figure \@ref(fig:bitriangles). In fact, the reason why these are called **interlock** networks, is because it is easy to see that, ultimately, by virtue of sharing members across boards, most big corporations in the U.S. (and other countries), end up [forming part of a single giant network](https://en.wikipedia.org/wiki/Interlocking_directorate).

# The Affiliation Matrix

```{r twomodeclubs, echo=FALSE, fig.margin = TRUE, fig.cap="Bipartite graph of a two-mode network of students and clubs.", fig.width = 5, fig.height=10}
...
Consider the **two-mode network** shown in Figure \@ref(fig:twomodeclubs). This is an affiliation network meant to represent the memberships of six students in five college activity clubs. As discussed earlier, we use a **bipartite graph** to represent the network. The bipartite graph represents the two sets of nodes using different shapes or colors (blue and red nodes in Figure \@ref(fig:twomodeclubs)), and draws a link between the people and the group if the person is affiliated with the group. 
...
```{r affmat, echo=FALSE}
    ...
    kbl(gr, caption = "Affiliation Matrix of the bipartite graph shown above.")
```
...
```{r affdegcent1}
    ...
    gr <- as_incidence_matrix(gr)
    d <- t(rowSums(gr))
    kbl(d, caption = "Degree centrality scores for the people.")
```
...
## Group Centralities
In the same way, if wanted to compute the degree centralities of other mode (the club node set, abbreviated as *G*), then we would calculate the *column sums* of the affiliation matrix using a slight variation of equation \@ref(eq:aff1), like we did when we switched from outdegree to indegree:
...
```{r affdegcent2}
    ...
    gr <- as_incidence_matrix(gr)
    d <- t(colSums(gr))
    kbl(d, caption = "Degree centrality scores for the clubs")
```
...
Using the rules for matrix multiplication discussed in the lesson on matrices, the person overlap matrix obtained using the affiliation matrix shown in Table \@ref(tab:affmat) is shown in Table \@ref(tab:poverlap).
...
```{r poverlap}
    ...
    gr <- as_incidence_matrix(gr)
    c <- gr %*% t(gr)
    kbl(c, caption = "Person Overlap Matrix.")
```
...
Note also that, in the person overlap matrix, (in contrast to the usual adjacency matrix), there are valid entries along the diagonal cells ($o^P_{ii}$). These cells now record the *total number of memberships* that the node corresponding to that row (or column) has. Which we ascertained by computing the node centralities in the original affiliation matrix using \@ref(eq:aff1). You can see that the vector of degree centralities shown in Table \@ref(tab:affdegcent1) is the same as the vector formed by the diagonal entries in Table \@ref(tab:comem).
...
Recall from the lesson on matrices that matrix multiplication is *not* commutative (if $A$ is a rectangular matrix, then $A \times A^{'} \neq A^{'} \times A$)), so equation \@ref(eq:goverlap) gives you a different answer than equation \@ref(eq:poverlap). The result is shown in Table \@ref(tab:goverlap)

```{r goverlap, fig.margin= TRUE}
    clubs <- c("Super Smash Brs.", "Cheese", "Nerdfighters", "Magic", "Fashion")
    fr <- c(rep("Gabriela", 4), rep("Parker", 2), rep("Brandon", 3), rep("Marie", 2), rep("Rahul", 2), rep("Minjoo", 1))
    set.seed(7834)
    to <- c(sample(clubs, 4), sample(clubs, 2), sample(clubs, 3), sample(clubs, 2), sample(clubs, 2), sample(clubs, 1))
    ...
    gr <- as_incidence_matrix(gr)
    c <- t(gr) %*% gr
    kbl(c, caption = "Group Overlap Matrix.")
```
...
For instance, in Figure \@ref(fig:twomodeclubs), Gabriela's node neighborhood is: 
...
```{r onemode1, fig.margin = TRUE, fig.cap = "One mode (persons) projection of the original bipartite graph.", fig.width = 8, fig.height = 5}
    clubs <- c("Super Smash Brs.", "Cheese", "Nerdfighters", "Magic", "Fashion")
    fr <- c(rep("Gabriela", 4), rep("Parker", 2), rep("Brandon", 3), rep("Marie", 2), rep("Rahul", 2), rep("Minjoo", 1))
    set.seed(7834)
    to <- c(sample(clubs, 4), sample(clubs, 2), sample(clubs, 3), sample(clubs, 2), sample(clubs, 2), sample(clubs, 1))
...
So using formulas \@ref(eq:poverlap) and \@ref(eq:goverlap), it is possible to go from a two-mode network in which no links exist between nodes of the same kind, to a weighted graph, in which the links between nodes of the same kind are defined by the overlap of their neighborhoods in the original bipartite graph. As we noted earlier, this is called the **one mode projection** of the two-mode network. Each two-mode network thus has two one mode projection one for each node set.

```{r onemode2, fig.margin = TRUE, fig.cap = "One mode (groups) projection of the original bipartite graph.", fig.width = 8, fig.height = 5}
    clubs <- c("Super Smash Brs.", "Cheese", "Nerdfighters", "Magic", "Fashion")
    fr <- c(rep("Gabriela", 4), rep("Parker", 2), rep("Brandon", 3), rep("Marie", 2), rep("Rahul", 2), rep("Minjoo", 1))
    set.seed(7834)
    to <- c(sample(clubs, 4), sample(clubs, 2), sample(clubs, 3), sample(clubs, 2), sample(clubs, 2), sample(clubs, 1))
...
The one mode projection for the person node set of the bipartite graph show in Figure \@ref(fig:twomodeclubs) is shown in Figure \@ref(fig:onemode1), this is an **undirected weighted graph** with the edge weight between people being set to the number of comemberships between each dyad as recorded in the person overlap matrix shown in Table \@ref(tab:poverlap). In this respect, the number of comemberships can be seen as a proxy of the **tie strength** between two people, when we only have information on their affiliations. As the Figure shows, Brandon, Gabriela and Rahul form a tightly connected clique, given the number of memberships they share. Minjoo, who does not share many affiliations with anyone, stands toward the periphery of the person-to-person comembership network. 

The corresponding one-mode projection for the group node set is shown  in Figure \@ref(fig:onemode2). This weighted graph can be read the same way: The thickness of the ties between groups are proportion to the people they share as recorded in the group overlap matrix shown in Table \@ref(tab:goverlap), thus speaking to the *similarity* or *strength* of connectivity between groups. 
