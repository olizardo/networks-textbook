...
For instance, in @fig-connected1-2, there are two components, one formed by the connected subgraph with node set $\{A, B, C, D, E\}$, and the other formed by the connected subgraph with node set $\{F, G, H, I\}$. 
...
Let's apply these definitions to the simple directed graph shown in @fig-directed (from @sec-directedgraphs):
- **Is it strongly connected?** No. For example, node $C$ is a receiver node with $k_{out} = 0$. Since it has no outgoing connections, once you reach $C$, you cannot get to any other node in the graph. Likewise, node $E$ has $k_{in} = 0$, meaning no other node can ever reach $E$. Thus, it is not strongly connected.
- **Is it weakly connected?** Yes. If we replace all directed arrows in @fig-directed with simple undirected lines, we get a single, fully connected undirected graph where every node can reach every other node.
