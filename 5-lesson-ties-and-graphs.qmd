...
## Anti-Symmetric Ties and Tree Graphs {#sec-tree}
...
One feature of a network composed of only anti-symmetric relations is that its corresponding graph can always be drawn from top to bottom, starting (at the top) with the node that only sends but does receive any ties and ending (at the bottom) with nodes that only receive, but do not send, ties. This is called a **tree graph** and an example is shown in @fig-tree. As noted, your family tree is an example of a tree graph of anti-symmetric kin ties going from people in an older generation to those in a younger generation. For instance, *A* could be your grandmother, and *B*, *C*, and *D* could be her three daughters. If *B* was you mom, then you could be *E* (along with your siblings *F* and *G*) and your cousins *H*, *I*, *J*, *K*, *L*, *M*.

As we will see in @sec-indirect, when considering connections between people that involve intermediaries, there is a special pattern called a **cycle**. This involves a node sending a directed tie to another node, which sends a directed tie to a third node. What makes it a cycle is that third node then sends a direced tie to the first node we began with. Recall from @sec-dirtriads, that the three-person subgraph *AAA(C)* shown in @fig-AAAC forms a cycle. 

Well, a key property of tree graphs composed of anti-symmetric ties is that *they can't have any cycles*. No node that sends a tie can receive a tie back! This is the reason why tree graphs are said to to be "acyclic" (they lack cycles) and also why they go by the name **directed acyclic graphs** which is sometimes abbreviated as "DAG." DAGs have applications in certain fields of statistics, causal analysis, and machine learning and have been made famous by [Judea Pearl](http://bayes.cs.ucla.edu/jp_home.html), a Professor of Computer science right here at UCLA.
