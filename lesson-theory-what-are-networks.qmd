...
For instance, some social networks do not even have to be made up of real people! In fictional worlds, such as the *Game of Thrones* HBO series created from the books written by George R. R. Martin, some characters meet and get to know one another, but others never meet, or more likely in this show, (spoiler!) *die* in some kind of gruesome fashion before they get to meet. So we can construct a social network of *acquaintance relations* among the characters (who ended up meeting whom), as depicted in @fig-got. This is a **social network** because the links are composed of a psychological or emotional relation between people. The emerging fields of the [https://en.wikipedia.org/wiki/Digital_humanities](digital_humanities) studies these kinds of networks, which are often constructed from historical or literary sources. 

In the case of GoT, the relation "knowing somebody" in Martin's fictional world is an example of a **symmetric tie** between people. In @fig-got, the size of the name of each character is proportional to the **centrality** of the actor in the network. As we will see in @sec-bigthree, centrality is an index of how *important* a given actor is in a network. So, even if you've never watched the show, you'll know that Tyrion, Daenerys, and Jon are pretty important people in this show!
...
Other networks are composed of links between physical technological systems, not people. Take, for instance, the various airports (large and small, international and regional) in the United States. Every day, a number of flights depart from one airport and land at another. This means that airports in the US are linked via **directed** (e.g., from/to) ties in a socio-Technical network. This tie is directed because it goes *from* one airport (where the flight departs) *to* another airport (where the flight lands). The network of airports in the US, corresponding to the local carrier *Southwest*, is shown in @fig-airport, with the direction of the lines of omitted. Like in GoT the size of the nodes is scaled to the *centrality* of the airport in the network. The larger the node, the more central the airport is in the network. Which gives you a sense of where Southwest "hubs" are located (hint a lot of them are in the Southwest!).

This network is a **socio-technical network** because it involves both **social** and **technological** links built from flights that connect airports (see @fig-airport). The network is **social** because it is composed of people traveling. The network is also **technological** because its links are made possible by a complex web of air-flight technology, including planes, radars, in-flight computer systems, and so forth. As we will see in @sec-ties, the directed links between airports are an example of an **asymmetric tie**; some airports fly frequently to other airports (e.g., Small Regional to LAX), but other airports (e.g., LAX) only schedule flights to major airports. 
...
All of us carry around a very complex and staggeringly large network, composed of millions of nodes and billions of connections. The network is called *the brain* (see @fig-brain). The nodes are a special type of biological cell called a *neuron*. Neurons are special because they have these filaments called *dendrites* and a long body called an *axon*. The axon of one neuron links up to the dendrites of another one, generating a large-scale complex network that allows you to breathe, eat, drink, think, see, smell and read these pages. Surprisingly, for a long time people *did not know* that the brain was a network, because they didn't know neurons existed (they thought it was all a big blob). It wasn't until the Spanish neuroscientist [Santiago Ramón y Cajal](https://en.wikipedia.org/wiki/Santiago_Ram%C3%B3n_y_Cajal) (1852-1934) used a special staining technique to visualize neurons that people realized the brain was a network!

But the brain is not the only network you carry around. In fact, YOU, your whole body, can be thought of as a network, at least according to network biologists [@lehnertz_etal20]. For instance, as you can see in @fig-body, we can think of the body as a network, with organs as nodes and physiological connections as connections. It turns out that the song about all the bones being connected to one another wasn't that far off from reality. 

![The Human body as a network *via* [https://doi.org/10.3389/fphys.2020.598694](https://doi.org/10.3389/fphys.2020.598694)](images/body.jpg){#fig-body width=300}
...
@fig-symptoms shows a psychological network of symptoms, taken from [Diagnostic and Statistical Manual of Mental Disorders](https://www.psychiatry.org/psychiatrists/practice/dsm) (DSM for short), created by psychological scientists who study psychopathology. In these networks, the symptoms that cluster together (shown as nodes of different colors) come to define specific types of mental disorders, like "Social Phobia" or "Panic Disorders."
