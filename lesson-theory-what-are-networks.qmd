...
All of us carry around a very complex and staggeringly large network, composed of millions of nodes and billions of connections. The network is called *the brain* (see @fig-brain). The nodes are a special type of biological cell called a *neuron*. Neurons are special because they have these filaments called *dendrites* and a long body called an *axon*. The axon of one neuron links up to the dendrites of another one, generating a large-scale complex network that allows you to breathe, eat, drink, think, see, smell and read these pages. 

But the brain is not the only network you carry around. In fact, YOU, your whole body, can be thought of as a network, at least according to network biologists [@lehnertz_etal20]. For instance, as you can see in the figure below, we can think of the body as a network, with organs as nodes and physiological connections as connections. It turns out that the song about all the bones being connected to one another wasn't that far off from reality. 
