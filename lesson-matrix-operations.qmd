...
## Matrix Algebra of the Dyad Census {#sec-matrix-dyad-census}
...
As covered in @sec-ties and @sec-cliques, under asymmetric relations, any pair of nodes (a **dyad**) can have one of three mutually exclusive configurations:
...
3.  **Null (N)**: Neither directed edge is present (completely disconnected).
...
### 1. The Mutual Dyads Matrix ($\mathbf{M}$)
...
$$
\mathbf{M} = \mathbf{A} \circ \mathbf{A}^T
${#eq-matrix-mutual}
...
$$
N_M = \frac{\sum_{i,j} M_{ij}}{2}
${#eq-count-mutual}

### 2. The Asymmetric Dyads Matrix ($\mathbf{S}$)
...
$$
\mathbf{S} = \mathbf{A} \circ (\mathbf{1} - \mathbf{A}^T)
${#eq-matrix-asym}
...
### 3. The Null Dyads Matrix ($\mathbf{N}$)
...
$$
\mathbf{N} = (\mathbf{1} - \mathbf{A}) \circ (\mathbf{1} - \mathbf{A}^T)
${#eq-matrix-null}
...
Let us calculate the matrix dyad census for a 6-node directed reference graph $\mathbf{A}$ using these algebraic steps. The reference adjacency matrix $\mathbf{A}$ of this network is shown in @tbl-adj-ref:
...
: Directed Reference Adjacency Matrix $\mathbf{A}$ {#tbl-adj-ref}

#### 1. Calculating the Mutual Matrix ($\mathbf{M}$)

Applying the Hadamard product between $\mathbf{A}$ and its transpose $\mathbf{A}^T$, we obtain the mutual dyads matrix $\mathbf{M}$ shown in @tbl-mutual-ref:
...
: Mutual Dyads Matrix $\mathbf{M} = \mathbf{A} \circ \mathbf{A}^T$ {#tbl-mutual-ref}

Summing all elements in @tbl-mutual-ref, we get $4$. Plugging this into @eq-count-mutual gives the count of mutual dyads:
$$
N_M = \frac{4}{2} = 2\text{ mutual dyads (A-C and E-F)}
$$

#### 2. Calculating the Asymmetric Matrix ($\mathbf{S}$)

Multiplying $\mathbf{A}$ element-wise by the complement of its transpose $(\mathbf{1} - \mathbf{A}^T)$ yields the asymmetric matrix $\mathbf{S}$ shown in @tbl-asym-ref:
...
: Asymmetric Dyads Matrix $\mathbf{S} = \mathbf{A} \circ (\mathbf{1} - \mathbf{A}^T)$ {#tbl-asym-ref}

Summing all the elements in @tbl-asym-ref, we obtain the count of asymmetric dyads:
$$
N_A = 8\text{ asymmetric dyads}
$$

#### 3. Calculating the Null Matrix ($\mathbf{N}$)

Finally, multiplying the complement of $\mathbf{A}$ by the complement of its transpose $\mathbf{A}^T$ yields the null matrix $\mathbf{N}$ shown in @tbl-null-ref:
...
: Null Dyads Matrix $\mathbf{N} = (\mathbf{1} - \mathbf{A}) \circ (\mathbf{1} - \mathbf{A}^T)$ {#tbl-null-ref}

Summing only the off-diagonal cells in @tbl-null-ref, we find $10$ ones. Plugging this into @eq-count-null gives the count of null dyads:
$$
N_N = \frac{10}{2} = 5\text{ null dyads}
$$

### The Matrix Dyad Identity Check
...
$$
\mathbf{M} + \mathbf{S} + \mathbf{S}^T + \mathbf{N} = \mathbf{J} - \mathbf{I}
$${#eq-matrix-identity}
