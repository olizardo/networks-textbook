...
p1 <- ggraph(gr1 <- tbl_graph(edges = as_data_frame(gr1), nodes = as_data_frame(gr1)), layout = l) +
...
p1
   ...
   gr2 <- play_islands(4, 3, m = 1, p_within = 1) %>% 
     activate(nodes) %>% 
     mutate(name = toupper(letters[1:12]))
p2 <- ggraph(gr2 <- tbl_graph(edges = as_data_frame(gr2), nodes = as_data_frame(gr2)), layout = l) +
