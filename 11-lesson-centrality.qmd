...
This information is captured in a network's geodesic **distance matrix**, as discussed in @sec-specmats. For instance, take the graph shown in @fig-clos, the distance matrix for which is shown in @tbl-dis. 
...
In @eq-closcen, the denominator is the sum across each column *j*, for each row *i* in @tbl-dis which corresponds to the distance between node *i* and each of the other nodes in the graph *j* (skipping the diagonal cell when $i=j$, because the geodesic distance of node to itself is always zero!). We take the mathematical *inverse*, dividing the sum of the distances by the number of nodes in the graph minus one, so that way, the smallest number comes out on top and the bigger number comes out of the bottom (since, as we said, we want to measure *closeness* not "farness.") 

Applying @eq-closcen to the distance matrix shown in @tbl-dis, results in the closeness centrality scores for each node shown in @tbl-clos. 

```{r}
#| label: tbl-clos 
#| tbl-cap: "Closeness centrality scores for an undirected graph."
    ...
    d <- distances(gr)
    C <- nrow(d) - 1
    d <- round(C / rowSums(d), 2)
    kbl(d, format = "html", align = "c", padding = 2,
        col.names = c("Closeness Centrality")) %>% 
        row_spec(5, background = "salmon", bold = TRUE) %>% 
        row_spec(14, background = "lightgreen", bold = TRUE) %>% 
...
So far, so good. However, what would happen if we tried to compute closeness centrality for a **disconnected graph** like the one shown in Figure @fig-connected1-2? Well, the shortest paths distance matrix for that graph looks like this:
...
Note that pairs of nodes that cannot **reach** one another in the disconnected graph, get a geodesic distance of "Inf" (infinity) in the respective cell of the geodesic distance matrix. This is a problem because when we compute the row sums of the geodesic distance matrix to try to calculate centrality according to @eq-clos, we get:

```{r}
    d <- t(rowSums(d))
    kbl(d, format = "pipe", align = "c") 
```
Not to get too philosophical, but the problem is that when you add any number to "infinity," the answer is, well, *infinity*^[see [https://byjus.com/maths/infinity/](https://byjus.com/maths/infinity/)] This means that closeness centrality is only defined for **connected graphs**. When it comes to disconnected graphs, we are out of luck. 

Thankfully, there is a solution develoed by @rochat09. It consists of a modification of @eq-closcen called **harmonic closeness centrality**. The formula goes as follows:
...
Now, this might seem like we just re-arranged the stuff in @eq-closcen, and indeed that's what we did! But the re-arrangement matters, because it changes the *order* in which we do the various arithmetic operations. 

So, in English, while @eq-closcen says "*first* sum the geodesic distances for each node (to get the denominator), and *then* divide $N-1$ by this sum," @eq-harmcen says "*first* divide one by the geodesic distance, and *then* sum the result of all these divisions, and *then* multiply this sum by one over $N-1$. 

Once again, the philosophy of infinity kicks in here, since the main difference is that one divided by infinity is actually a real number: **zero**.^[Once again, see [https://byjus.com/maths/infinity/](https://byjus.com/maths/infinity/)] 
...
Beautiful! Now, instead of weird "Inf"s we have zeroes, which is great because we can add stuff to zero and get a real number back. We can then apply @eq-harmcen to the numbers in @tbl-dis2 to get the **harmonic closeness centrality** for each node in @fig-connected1-2. These are shown in @tbl-clos2.

```{r}
#| label: tbl-clos2 
#| tbl-cap: "Closeness centrality scores for nodes in a disconnected, undirected graph."
#| tbl-cap-location: margin
