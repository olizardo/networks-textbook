...
### Nodes
The set of nodes usually represents actors in the real-world social network. **Point and line diagrams** (such as the ones shown in @fig-karate) are used to represent graphs, which in turn represent the real social network.
...
-   In a graph, if two nodes are joined together by an edge, they are said to be **adjacent**. So in  @fig-simple(a), nodes *A* and *B* are adjacent, as are nodes *A* and *C*. Pairs of nodes that are not linked by an edge, like nodes *B* and *C*, are said to be **nonadjacent**.
...
-   If an edge "touches" a node (e.g., connects it to another node), we say that that edge is **incident** on that node. So in @fig-simple(a), the edge *AB* is incident on both nodes *A* and *B*. The relation of **incidence** will be important in a later lesson when we discuss node-level network metrics, such as **degree centrality**.
...
- If a node is connected to $n-1$ nodes in a graph---basically, every other node but themselves---it is called a **star node** or a **dominant node**. The star node is the opposite of an isolate, having the maximum number of connections observed in a social network. 
