# Equivalence and Similarity {#sec-structequiv}
...
```{r}
#| label: fig-three-equiv-base
#| fig-cap: "A symmetric 15-node undirected graph representing a classic role-structure motif."
#| fig-cap-location: bottom
#| fig-height: 6
#| fig-width: 10
    ...
    # Build the 15-node adjacency matrix
    adj <- matrix(0, 15, 15)
    adj[2, c(1, 3, 4, 5)] <- 1
    adj[c(1, 3, 4, 5), 2] <- 1
    adj[2, 7] <- 1
    ...
    gr_base <- graph_from_adjacency_matrix(adj, mode = "undirected")
    V(gr_base)$name <- toupper(letters[1:15])
    
    # Custom layout to highlight the symmetry of the star hubs and the central core
    coords <- matrix(0, 15, 2)
    coords[2, ] <- c(-3, 0) # Hub B
    coords[1, ] <- c(-4, 1) # Leaves of B
    coords[3, ] <- c(-4, -1)
    coords[4, ] <- c(-4.5, 0.5)
    coords[5, ] <- c(-4.5, -0.5)
    
    coords[7, ] <- c(-1, 0) # Bridge G
    coords[6, ] <- c(0, 1)  # Top F
    coords[8, ] <- c(-0.5, -0.5) # Core H
    coords[9, ] <- c(0.5, -0.5)  # Core I
    coords[10, ] <- c(1, 0)  # Bridge J
    
    coords[11, ] <- c(3, 0) # Hub K
    coords[12, ] <- c(4, 1) # Leaves of K
    coords[13, ] <- c(4, -1)
    coords[14, ] <- c(4.5, 0.5)
    coords[15, ] <- c(4.5, -0.5)

    p_base <- ggraph(gr_base, layout = coords) +
...
Because of this strict "same-neighbor" rule, structural equivalence is a highly restrictive concept. In our 15-node network (@fig-three-equiv-base), only the non-adjacent peripheral leaf nodes can share exactly the same neighborhoods:
- Leaf nodes $\{A, C, D, E\}$ are structurally equivalent because all of them are connected *only* to hub $B$.
- Leaf nodes $\{L, M, N, O\}$ are structurally equivalent because all of them are connected *only* to hub $K$.
...
In our 15-node network (@fig-three-equiv-base), we can observe a perfect left-to-right bilateral symmetry. If we fold the network in half across the central vertical axis, hub $B$ aligns with hub $K$, the bridge $G$ aligns with bridge $J$, and the leaves of $B$ align with the leaves of $K$. This symmetry yields **five automorphic equivalence classes**:
1. **The Hubs**: $\{B, K\}$ (the two central star hubs).
2. **The Leaves**: $\{A, C, D, E, L, M, N, O\}$ (all peripheral leaf nodes).
3. **The Core Bridges**: $\{G, J\}$ (the core nodes that act as gates to the hubs).
4. **The Core Mid-points**: $\{H, I\}$ (the core nodes that represent central symmetric nodes).
5. **The Core Top**: $\{F\}$ (the central core top node, which is symmetric only to itself).
...
In our 15-node network, regular equivalence collapses the five automorphic classes into **four regular equivalence classes** (or social roles):
1. **The Leaves**: $\{A, C, D, E, L, M, N, O\}$. (All are regularly equivalent because they connect *only* to a Hub).
2. **The Hubs**: $\{B, K\}$. (They connect to Leaves and Core-Bridges).
3. **The Core-Bridges**: $\{G, J\}$. (They connect to Hubs and Core nodes).
4. **The Core Nodes**: $\{F, H, I\}$. (They connect only to other Core nodes and Core-bridges).
...
### The Nested Hierarchy of Equivalence
...
This nesting means that:
- Every pair of nodes that is **structurally equivalent** is also **automorphically equivalent** (if you share neighbors, you can be swapped without changing the graph).
- Every pair of nodes that is **automorphically equivalent** is also **regularly equivalent** (if you can be globally swapped, you must connect to role-equivalent partners).
- However, the reverse is not true. Regular equivalence is the most general and flexible, automorphic is intermediate, and structural is the most strict and restrictive.

@fig-equiv-comparison visualizes these three distinct partitions on the same 15-node network, illustrating how network analysts zoom in and out of relational role structures.

```{r}
#| label: fig-equiv-comparison
#| fig-cap: "Comparison of the three equivalence partitions on the 15-node network: (a) Structural Equivalence, (b) Automorphic Equivalence, and (c) Regular Equivalence."
#| fig-subcap:
#|   - "Structural Equivalence (Identifies leaf sharing, leaving hubs and core in unique positions)"
#|   - "Automorphic Equivalence (Identifies bilateral symmetry, grouping hubs and symmetric core nodes)"
#|   - "Regular Equivalence (Identifies four clean social roles based on connection patterns)"
#| layout-ncol: 1
#| fig-height: 6
#| fig-width: 10
    ...
    p_structural <- ggraph(gr_base, layout = coords) +
      geom_edge_link(color = "steelblue", alpha = 0.5, edge_width = 1.0) +
      geom_node_point(aes(color = structural), size = 18) +
      geom_node_text(aes(label = name), size = 8, fontface = "bold", color = "white") +
    ...
    p_automorphic <- ggraph(gr_base, layout = coords) +
      geom_edge_link(color = "steelblue", alpha = 0.5, edge_width = 1.0) +
      geom_node_point(aes(color = automorphic), size = 18) +
      geom_node_text(aes(label = name), size = 8, fontface = "bold", color = "white") +
    ...
    p_regular <- ggraph(gr_base, layout = coords) +
      geom_edge_link(color = "steelblue", alpha = 0.5, edge_width = 1.0) +
      geom_node_point(aes(color = regular), size = 18) +
      geom_node_text(aes(label = name), size = 8, fontface = "bold", color = "white") +
