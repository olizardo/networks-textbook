---
title: "Types of Ties and Their Graphs"
---
...
As noted in @sec-graphtheory, Nodes and edges are indeed the building blocks of a graph. However, the types of relationships that the edges represent can change both how we understand the network conceptually and what mathematical techniques we can apply to the graph when computing *graph metrics* (the subject of @sec-graphmetrics). The basic idea is that when we do network analysis, we want to map our understanding of the nature of the **social relationships** we are studying to the **types of graphs** we use to represent the network formed by the **concatenation** of those relationships [@martin09]. 

In @sec-types, we began discussing how sociologists and graph-theory people classify ties and how they could be represented using different graphs. In this chapter, we elaborate on these considerations and provide examples. 
...
Let us assume that @fig-undirected represents a network of people who *spend time together* (what we called a **similarity** in @sec-types). One way to build this network would be to ask people on your dorm floor who they spend some time (e.g., more than an hour a week) hanging out with. By definition, the relation "spending time together" lacks any inherent directionality. **Mutuality** (or **reciprocity**) is built in by construction. It would be nonsensical for a person (say *A*) to claim that they spend time with another person (say *B*) and for *B* to say that they do not spend time with *A*. In social network analysis, these types of ties are called **symmetric ties** [@heider46]. So if $AB$ means that *A* spends time with *B*, if we know $AB$ is true, then we know that $BA$ is also true. 

In the same way, two people being in the same place at the same time (co-location), even if they do not know one another, constitutes a symmetric tie. You also have the symmetric tie of "being in the same class as" every other student taking your Social Networks seminar this term. Note that, in this sense, all **co-memberships** create symmetric ties among all involved actors (we will revisit this topic when discussing **affiliation networks** in @sec-affiliation). For instance, if I am a member of your family, you are also my family member; if we are both members of the soccer club, we are considered teammates. 
...
Networks composed of symmetric ties have some interesting properties. If we know that the relationship (**R**) linking two nodes *A* and B is **symmetric**, then only a single edge exists that links them, and it does not matter whether we call this edge *AB* or *BA*. The order does not matter. In this way, we can formally define a **symmetric tie** as one that lacks directionality; if a tie is symmetric, then if we know that A is related to B (the AB edge is part of the edge set of the graph), then we know by necessity that B is related to A. 
...
In contrast to spending time together, being members of the same family, or being in the same place at the same time, some social ties allow for inherent **directionality**. Edges in these graphs are called **asymmetric ties** [@heider46]. That is, one member of the pair can claim to have a particular type of social relationship with the other, but it is possible (though not necessary) that the other person does not have the same relationship with the first. 

Helping or **social support** relations are like this. For instance, you can help someone with their homework, or give them personal advice, but this does not *necessarily* mean that that person will return the favor. They may, or they may not. The point is that, in contrast to a symmetric tie, **mutuality** or **reciprocity** is not built in by definition, but must happen as an empirical event in the world. We need to ask the other person to find out (or check their email logs). Can you think of other examples of asymmetric social ties?
...
**Reciprocity** is an important concept in social network analysis. Some have said it is perhaps *the most* important concept for understanding human society [@gouldner60], which may be an exaggeration. Only asymmetric ties may have the property of being non-reciprocal or having more or less reciprocity. If I think you are my friend, I very much hope that you also think you are my friend. 
...
For some ties, such as advice, support, or friendship, reciprocity is all-or-none; it either exists or it does not. For instance, the friendship offer you extend to someone may be reciprocated (or not). In the same way, you can like someone, and they may like you back (or not), like the notes you passed around in middle school. For other ties, such as communication ties (e.g., those defined by the amount of texting or calling), reciprocity is a matter of degree; there may be more or less. For instance, you can text someone 10 times a day, but they may text you back only half the time. In all cases, reciprocity is at its maximum when the content of the relationship is exchanged equally between actors. 

Can you think of relationships in your life characterized by more or less reciprocity?

Just like symmetric ties are represented using a particular type of graph (namely, an **undirected graph**), social networks composed of asymmetric ties are best represented by a type of graph called a **directed graph** (or **digraphs** for short). @fig-directed shows the point-and-line diagram of a digraph. What were simple lines for in the undirected graph shown in @fig-undirected have been replaced with arrows indicating directionality. 

A node **sends** a relationship to the node that the arrow points to, which in turn *receives* the relationship. In a digraph, up to two directed arrows may link nodes, allowing them to go in both directions. When an undirected graph has no **loops** (edges connecting a node to itself), and there is only one edge connecting a sender node to a receiving node (the graph has no **multiedges**), like @fig-directed, it is called a **simple directed graph**. 

In a directed graph, for every edge, there is a **source node** and a **destination node**. So in the case of "A helps B," the source node is *A* and the destination node is *B*. In the case of "B helps A," the source node is *B* and the destination node is *A*. This means that in a directed graph, in contrast to an undirected one, *the order in which you list the nodes when you name the edges matters*. Thus, the edge *AB* is a different one from the edge *BA*. The first one may exist, but the second one may not exist (edges in a directed graph are sometimes also called **arcs**). 

For instance, if @fig-directed were an **advice network** [@cross01], we could say that *H* seeks advice from *D*, but *D* does not seek advice from *H*. This may be because *D* is higher in the office hierarchy or more experienced than *H*, in which case a lack of reciprocity may indicate an authority relationship between the two nodes. 
...
Although not a particularly important social relationship, *being taller than someone* is an anti-symmetric tie. If you are taller than someone else, they cannot be taller than you. If you are taller than someone else, that means the "being taller than" link has you as the source and the person you are taller than as the destination, but it can never go the other way around. However, if someone else* is taller than you, then that link will have them as the source and you as the destination. 

More sociologically interesting types of anti-symmetric links in social networks include **authority relations**, such as hierarchical ties at work, and **cross-generational relations**, such as kinship ties in families. For instance, the authority relation *gives orders to* can only go in one direction. Imagine an organization where the boss tells you what to do, and you, in turn, tell your boss what they should do. It would be pretty anarchic. In the kinship ties domain, the relationship *being the parent of* is a classic anti-symmetric tie. If you are a parent, you cannot, in turn, be your own parent. 
