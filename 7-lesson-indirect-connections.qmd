...
#| fig-cap: "A connected versus disconnected graph."
#| fig-cap-location: margin
...
When a graph goes from connected (like $G(A)$ in @fig-connected1 to disconnected (like $G(B)$ @fig-connected1 it is split into a number of subgraphs that are themselves connected. For instance, in the $G(B)$ shown in @fig-connected1 the set nodes $\{A, B, C, D, E\}$ form a **connected subgraph** of the larger disconnected graph. A connected subgraph that is part of a larger disconnected graph is called a **component**.  In the $G(B)$ graph in @fig-connected1 there are two components, one formed by the connected subgraph with node set $\{A, B, C, D, E\}$, and the other formed by the connected subgraph with node set $\{F, G, H, I\}$. 
...
Note that we went from the connected graph shown as $G(A)$ in @fig-connected2 by removing or deleting particular edges (like the red and purple ones in $G(A)$). These edges are clearly more important than the blue ones, because they are responsible for keeping the whole graph together as a connected structure. Edges whose removal results in a graph going from being connected to disconnected are so important that they have a special name: **bridges**.

Thus, in a graph the bridges are (smallest in terms of cardinality) set of edges whose deletion results in the graph being disconnected. For instance, in graph $G(A)$ from @fig-connected1 the $DF$ edge is a bridge. In graph $G(A)$ from @fig-connected2), on the other hand, any two of the threesome formed by the two red edges and the purple edge count as bridges. Deleting any two of these three edges disconnects the graph. 

Note that in contrast to $G(A)$ from @fig-connected1, the graph $G(A)$ from @fig-connected2 doesn't have any *one* edge that is *the* bridge. Instead, it takes more the deletion of more at least two edges from the red and purple set to disconnect it. For instance, if we were to remove just the purple edge, the $G(A)$ would still be connected, and it would look like @fig-connected3), which is a connected graph. The same goes for removing just one of the red edges. If were to do that, we would end up with the graph shown in @fig-connected4), which is also a connected graph.
...
Sometimes, as with actors *A* and *B* in @fig-paths1), there will be **multiple shortest paths** between two pairs of nodes, because we end up with two or more paths that are "tied" in length (and thus all count as "shortest" paths). So for nodes *A* and *B* @fig-paths1 the two shortest paths connecting them are of length 3: $(AC, CE, EB)$ and $(AD, DE, EB)$.
