# Homework VIII: Balance and Signed Graphs {.unnumbered}
...
#| fig-width: 12
     set.seed(4567)
     n <- 9
     gr <- sample_gnp_signed(n, p = 0.25, p_neg = 0.45, 
...
```{r}
   block_model <- signed_blockmodel(gr, k = 2, alpha = 0.5, annealing = TRUE)
   n <- LETTERS[1:8]
```

- First clique:

`r n[which(block_model$membership == 1)]`


- Second clique:

`r n[which(block_model$membership == 2)]`
