...
This elegant restriction drastically simplifies the dyad census of anti-symmetric networks: every pair of nodes is either completely disconnected or connected by a strictly non-reciprocal arrow.
...
Under anti-symmetric ties, there are exactly **seven** permitted directed triad configurations:

1. **Null Triad** ($NNN$): All three dyads are completely disconnected.
2. **One-tie Triad** ($NNA$): Contains a single non-reciprocal directed tie and two disconnected dyads.
3. **Out-Star Triad** ($AAN(O)$): A single node sends non-reciprocal ties to both other nodes, while the third dyad is disconnected (e.g., $A \leftarrow B \rightarrow C$).
4. **In-Star Triad** ($AAN(I)$): Both other nodes send non-reciprocal ties to a single node, while the third dyad is disconnected (e.g., $A \rightarrow B \leftarrow C$).
5. **Directed Line / Transitive Path** ($AAN(L)$): Non-reciprocal ties form a directed path, while the third dyad is disconnected (e.g., $A \rightarrow B \rightarrow C$).
6. **Cycle Triad** ($AAA(C)$): A directed cycle of length three where non-reciprocal ties flow in a loop ($A \rightarrow B \rightarrow C \rightarrow A$).
7. **Transitive Triad / Hierarchy** ($AAA(I/O)$): The classic transitive triad where $A \rightarrow B \rightarrow C$ and also $A \rightarrow C$. This represents a perfectly consistent, transitive authority hierarchy.

This beautiful mathematical pruning simplifies the analysis of hierarchical systems. In formal organizations, for example, we expect to see transitive triads $AAA(I/O)$ (representing clean chains of command), while cycles $AAA(C)$ are structurally avoided as they introduce circular authority paradoxes.
...
```{r}
#| label: fig-NNA
#| fig-cap: "A disconnected directed triad with one non-reciprocal link."
#| fig-cap-location: bottom
#| fig-height: 3
#| fig-width: 3
...
```{r}
#| label: fig-NNM
#| fig-cap: "A disconnected directed triad with one reciprocal link."
#| fig-cap-location: bottom
#| fig-height: 3
#| fig-width: 3
...
```{r}
#| label: fig-AANL
#| fig-cap: "An open directed triad with a non-reciprocal directed line."
#| fig-cap-location: bottom
#| fig-height: 3
#| fig-width: 3
...
```{r}
#| label: fig-AANO
#| fig-cap: "An open directed triad with a non-reciprocal out-star."
#| fig-cap-location: bottom
#| fig-height: 3
#| fig-width: 3
...
```{r}
#| label: fig-AANI
#| fig-cap: "An open directed triad with a non-reciprocal in-star."
#| fig-cap-location: bottom
#| fig-height: 3
#| fig-width: 3
...
```{r}
#| label: fig-MANX
#| fig-cap: "Open directed triads with one reciprocal link."
#| fig-cap-location: bottom
#| fig-height: 6
#| fig-width: 3
...
```{r}
#| label: fig-MMN
#| fig-cap: "An open directed triad with two reciprocal links."
#| fig-cap-location: bottom
#| fig-height: 3
#| fig-width: 3
