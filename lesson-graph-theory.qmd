...
```{r}
#| label: fig-simple2
#| fig-cap: "A simple unlabeled graph with four nodes and two edges."
#| fig-cap-location: margin
...
This says that the graph *G* shown in @fig-simple1 is a set with two members, *E* and *V*, each of which is its own set. The edge set of *G* has two members, *AB* and *AC*. The node set of *G* has four members, *A*, *B*, *C*, and *D*. The number of members in a set is typically referred to as the **cardinality** of the set, written $|N|$ where $N$ is the name of the set. 

So, in this case, the cardinality of the edge set is two $|E| = 2$, and the cardinality of the vertex set is four $|V| = 4$. These two basic graph properties are so important, that they can serve as a "signature" for the graph. Thus, sometimes people will refer to a graph as $G(n, m)$ where $n$ is the cardinality of the vertex set (number of nodes) and $m$ is the cardinality of the edge set (number of links). Thus, the graph in @fig-simple1, can be referred to as a $G(4, 2)$ graph. 
...
Graphs like that shown in @fig-simple1 and @fig-simple2 are called **simple graphs**. There are two requirements for a graph to count as a simple graph:
...
## Graph Labeling
Note that in @fig-simple1, the nodes have letters which function as "names" for each. Thus, we can refer to node $A$, or node $B$ and so forth. These names are arbitrary, we can as well use numbers $\{1, 2, 3, ...\}$ or a combination of letters and numbers $\{V_1, V_2, V_3, ....\}$. When the nodes of a graph are distinguished from one another using names, the graph is said to be **labeled**. Sometimes, the names don't matter, so we don't specify them, as in @fig-simple2. In this case, the graph is said to be **unlabeled**. Graph metrics like the ones we will compute starting on @sec-graphmetrics, are the same regardless of whether the graph is labeled or not. Most of the example graphs we will be looking at are of the labeled kind.
