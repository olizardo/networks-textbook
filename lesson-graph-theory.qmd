...
#| fig-height: 8
    ...
    gr <- create_empty(4)
    gr <- gr %>% 
        bind_edges(data.frame(from = 4, to = c(1, 3))) %>% 
        mutate(name = toupper(letters[1:4])) 
    ...
    gr <- create_empty(4)
    gr <- gr %>% 
        bind_edges(data.frame(from = 3, to = c(1, 4))) %>% 
        mutate(name = toupper(letters[1:4])) 
    p <- ggraph(gr, layout = l) 
    p <- p + geom_edge_link(color = "steelblue", edge_width = 1.25) 
    p <- p + geom_node_point(aes(x = x, y = y), size = 22, color = "tan2") 
...
This says that the graph *G* shown in @fig-simple(a) is a set with two members, *E* and *V*, each of which is its own set. The edge set of *G* has two members, *AB* and *AC*. The node set of *G* has four members, *A*, *B*, *C*, and *D*. The number of members in a set is typically referred to as the **cardinality** of the set, written $|N|$ where $N$ is the name of the set.

So, in the @fig-simple(a) case, the cardinality of the edge set is two $|E| = 2$, and the cardinality of the vertex set is four $|V| = 4$. These two basic graph properties are so important, that they can serve as a "signature" for the graph. Sometimes people will refer to a graph as $G(n, m)$ where $n$ is the cardinality of the vertex set (number of nodes) and $m$ is the cardinality of the edge set (number of links). Thus, the graph in @fig-simple(a), can be referred to as a $G(4, 2)$ graph. Note that @fig-simple(a) this is not the only possible $G(4, 2)$ graph. @fig-simple(b) and @fig-simple(c) show two other possible variations of a $G(4, 2)$ graph. 

## Graph Labeling
In @fig-simple(a-c), the nodes have letters which function as "names" for each. Thus, we can refer to node $A$, or node $B$ and so forth. These names are arbitrary, we can as well use numbers $\{1, 2, 3, ...\}$ or a combination of letters and numbers $\{V_1, V_2, V_3, ....\}$. 

When the nodes of a graph are distinguished from one another using names, the graph is said to be **labeled**. Sometimes, the names don't matter, so we don't specify them, as in @fig-simple(d). In this case, the graph is said to be **unlabeled**. 
...
-   Finally, in a graph, nodes that are not connected to any other nodes are called **isolates**. This means that in @fig-simple(a), node *D* is an isolate because it has zero edges incident upon it.
